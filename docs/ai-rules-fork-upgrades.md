...
The workflow is pinned to `itl-main-5ae333ed-r18` at fork commit `841b30af5d87eb212f497754f1328b38146cb279`. Its exact upstream provenance is `refs/heads/main@5ae333ed49dc66989e305b286acc93691bb96926`. `templates/dependency-lock.json` is the single source of this tag, commit, upstream provenance, downstream revision `18`, and `compatibilityStatus=passed`; project templates, code, docs, and tests must agree with it.

Fork `main` mirrors upstream and is never consumed by installed projects. Downstream changes exist only on immutable release branches/tags; the current pair is `release/itl-main-5ae333ed-r18` / `itl-main-5ae333ed-r18`, which point to the same release commit. Older immutable releases remain published only for provenance. `r18` accepts upstream platform-capability discovery, metadata safety, the common platform resolver, Claude `rules-1c`, and OpenCode permission validation while retaining ITL lifecycle ownership, bridge commands, single-client installation, delegated MCP, the exact form-edit contract, and the existing project-memory policy.
...
Use `scripts/build-ai-rules-release.ps1` as the only normal downstream reconstruction path. `templates/ai-rules-overlay/sections.json` is a schema-2 path ledger: every upstream-changed path is classified as `take-upstream`, `keep-current`, or `resolved` with its upstream and result SHA-256; every permitted downstream-only path is also recorded. `templates/ai-rules-overlay/AGENTS.md` is the canonical compact root contract.

`-Mode Prepare` starts from a clean branch based directly on the exact upstream commit, copies only downstream-only paths, applies `keep-current` files and managed templates, and leaves `resolved` paths for ordinary Git editing. `-Mode Verify` (also available as the compatible `-CheckOnly` alias) validates the already committed candidate: exact path accounting, path hashes, managed template equality, a linear history without merge commits, no unresolved merge, and a clean tree. Git path discovery is NUL-delimited so Cyrillic, spaces, and unusual path characters remain unambiguous.
