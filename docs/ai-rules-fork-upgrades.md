...
The workflow is pinned to `itl-main-b4d9875b-r11` at fork commit `af82570afca06c40a9588c8a678bf3665bba4870`. Its exact upstream provenance is `refs/heads/main@b4d9875b15c6d93f493035aee51f077126e72a21`. `templates/dependency-lock.json` is the single source of this tag, commit, upstream provenance, downstream revision `11`, and `compatibilityStatus=passed`; project templates, code, docs, and tests must agree with it.

Fork `main` is the clean upstream snapshot. Downstream changes exist only on immutable branch/tag `release/itl-main-b4d9875b-r11` / `itl-main-b4d9875b-r11`, which point to the same qualified commit. Installed projects never consume moving `main`. Older immutable releases remain published only for provenance; `r11` supersedes `r10` with the compact root contract and reproducible overlay builder.
