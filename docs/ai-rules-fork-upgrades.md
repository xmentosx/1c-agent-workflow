...
The `develop` workflow is pinned to `itl-main-410951e7-r30` at fork commit `dfe4fd1250b0c14c83be9f2c16ef62d8fc521add`. Its exact upstream provenance is `refs/heads/main@410951e74fd3e6b7a763cf49757935b9a34d3f31`. `templates/dependency-lock.json` is the single source of this tag, commit, upstream provenance, downstream revision `30`, and compatibility state; project templates, code, docs, and tests must agree with it. Compatibility starts as `pending` on `develop` and changes to `passed` only through a separate evidence-backed promotion.

Fork `main` mirrors upstream and is never consumed by installed projects. Downstream changes exist only on immutable release branches/tags; the current pair is `release/itl-main-410951e7-r30` / `itl-main-410951e7-r30`, which point to the same release commit. Older immutable releases remain published only for provenance. `r30` retains the r29 grilling family and adds thin `/grill-me` and `/grill-with-docs` wrappers for Kilo, Claude Code, Cursor, OpenCode, Qwen, and Command Code; Codex, Kimi, Cline, and Pi continue to use their native skill or prompt surfaces without duplicated command behavior.
