...
The `develop` workflow is pinned to `itl-main-410951e7-r21` at fork commit `37362c6fa0e29b8aee0f70e01d85bf77e41cc683`. Its exact upstream provenance is `refs/heads/main@410951e74fd3e6b7a763cf49757935b9a34d3f31`. `templates/dependency-lock.json` is the single source of this tag, commit, upstream provenance, downstream revision `21`, and compatibility state; project templates, code, docs, and tests must agree with it. Compatibility remains `pending` because exact-commit workflow Full and Release were explicitly omitted for this develop publication.

Fork `main` mirrors upstream and is never consumed by installed projects. Downstream changes exist only on immutable release branches/tags; the current pair is `release/itl-main-410951e7-r21` / `itl-main-410951e7-r21`, which point to the same release commit. Older immutable releases remain published only for provenance. `r21` accepts the upstream correction for `&Вместо` versus `&ИзменениеИКонтроль`, retains the qualified r20 downstream contracts, and atomically synchronizes fork `main` during publication.
