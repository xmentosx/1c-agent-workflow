...
The `develop` workflow is pinned to `itl-main-410951e7-r23` at fork commit `609976be8fefdf1c0168c36ee92f4d985cfd2b09`. Its exact upstream provenance is `refs/heads/main@410951e74fd3e6b7a763cf49757935b9a34d3f31`. `templates/dependency-lock.json` is the single source of this tag, commit, upstream provenance, downstream revision `23`, and compatibility state; project templates, code, docs, and tests must agree with it. Compatibility starts as `pending` on `develop` and changes to `passed` only through a separate evidence-backed promotion.

Fork `main` mirrors upstream and is never consumed by installed projects. Downstream changes exist only on immutable release branches/tags; the current pair is `release/itl-main-410951e7-r23` / `itl-main-410951e7-r23`, which point to the same release commit. Older immutable releases remain published only for provenance. `r23` retains the qualified r22 behavior and makes form/template scaffolding repeatable while preserving authored content.
