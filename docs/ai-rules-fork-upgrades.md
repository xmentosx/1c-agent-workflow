...
The `develop` workflow is pinned to `itl-main-410951e7-r28` at fork commit `431b87a3f43e7b1d6b3ea6f5ae038978c504c879`. Its exact upstream provenance is `refs/heads/main@410951e74fd3e6b7a763cf49757935b9a34d3f31`. `templates/dependency-lock.json` is the single source of this tag, commit, upstream provenance, downstream revision `28`, and compatibility state; project templates, code, docs, and tests must agree with it. Compatibility starts as `pending` on `develop` and changes to `passed` only through a separate evidence-backed promotion.

Fork `main` mirrors upstream and is never consumed by installed projects. Downstream changes exist only on immutable release branches/tags; the current pair is `release/itl-main-410951e7-r28` / `itl-main-410951e7-r28`, which point to the same release commit. Older immutable releases remain published only for provenance. `r28` retains the r27 behavior, requires explicit standard row-set and row-order decisions for added tables or trees, and preserves explicit `false` values in `1c-form-edit`.
