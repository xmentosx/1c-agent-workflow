...
The `develop` workflow is pinned to `itl-main-410951e7-r26` at fork commit `3a21d9741509ef7f168feaa8eea016ee84156f57`. Its exact upstream provenance is `refs/heads/main@410951e74fd3e6b7a763cf49757935b9a34d3f31`. `templates/dependency-lock.json` is the single source of this tag, commit, upstream provenance, downstream revision `26`, and compatibility state; project templates, code, docs, and tests must agree with it. Compatibility starts as `pending` on `develop` and changes to `passed` only through a separate evidence-backed promotion.

Fork `main` mirrors upstream and is never consumed by installed projects. Downstream changes exist only on immutable release branches/tags; the current pair is `release/itl-main-410951e7-r26` / `itl-main-410951e7-r26`, which point to the same release commit. Older immutable releases remain published only for provenance. `r26` retains the qualified r25 behavior, adds an explicit managed-form context boundary contract, and coordinates repeated runtime defects as one bounded defect class with one mutating owner.
