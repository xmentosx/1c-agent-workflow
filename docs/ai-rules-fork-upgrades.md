...
The workflow is pinned to `itl-main-5ae333ed-r19` at fork commit `7952e7d9bb050d67e145c0136e87b6855c353f58`. Its exact upstream provenance is `refs/heads/main@5ae333ed49dc66989e305b286acc93691bb96926`. `templates/dependency-lock.json` is the single source of this tag, commit, upstream provenance, downstream revision `19`, and `compatibilityStatus=passed`; project templates, code, docs, and tests must agree with it.

Fork `main` mirrors upstream and is never consumed by installed projects. Downstream changes exist only on immutable release branches/tags; the current pair is `release/itl-main-5ae333ed-r19` / `itl-main-5ae333ed-r19`, which point to the same release commit. Older immutable releases remain published only for provenance. `r19` retains the qualified r18 upstream intake and additionally protects the root `.dev.env` from full and client-scoped removal even when a legacy manifest lacks `template: true`; existing dotenv bytes and drift metadata remain untouched.
