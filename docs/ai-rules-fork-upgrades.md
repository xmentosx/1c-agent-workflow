...
The workflow is pinned to `itl-main-5f3d3f0-r20` at fork commit `151aa980b5e99b3d129e974925e734d9ef0afa3e`. Its exact upstream provenance is `refs/heads/main@5f3d3f03b778d7de38cf2cfb18a20cf3e7ed79d8`. `templates/dependency-lock.json` is the single source of this tag, commit, upstream provenance, downstream revision `20`, and compatibility qualification; project templates, code, docs, and tests must agree with it.

Fork `main` mirrors upstream and is never consumed by installed projects. Downstream changes exist only on immutable release branches/tags; the current pair is `release/itl-main-5f3d3f0-r20` / `itl-main-5f3d3f0-r20`, which point to the same release commit. Older immutable releases remain published only for provenance. `r20` rebuilds directly from the new upstream, keeps the compact root as a verified projection through `root-contract.json`, preserves delegated MCP and `.dev.env` ownership, and adds lifecycle guards to the new DB/DT/web mutators.
