...
| STAB-06 | P2: ledger, refs, worktrees, and metrics need bounded hygiene. | Batches A, B, and D are implemented. Batch B adds strictly read-only disposition; batch D adds opt-in bounded access summary metrics without changing the legacy array response. Batch C and mutation-capable cleanup remain queued. | Batches A and B registered; batch D is integrated and pending `RegisterChange`. | Not published; user hold. | Not installed. | B has local disposition proof. D has 63/63 owner proof; existing stores without an activated terminal baseline return `terminal.complete=false` instead of scanning archival history. Candidate deletion is not enabled. |
