...
| STAB-06 | P2: ledger, refs, worktrees, and metrics need bounded hygiene. | Batches A-D are implemented. C adds a 2,048-entry hot run index with lifetime aggregates and content-addressed archival compaction for terminal resource records; mutation-capable ref cleanup remains queued. | Batches A-D registered; C Targeted passed 189/189 in 549.885 seconds at `bb30723`. | Not published; user hold. | Not installed. | C owner suites passed 51/51, 35/35, and 67/67 after independent GO. Registration exposed and fixed two PS5.1 empty-aggregate defects before queue movement. Raw run proofs remain immutable and exact lookup falls back fail-closed. Candidate deletion is not enabled. |
