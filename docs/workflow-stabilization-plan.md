...
| STAB-09 | P1: stale Pester estimates and serial-tail scheduling can exhaust the Targeted hard budget after every parallel owner has passed. | Implemented and independently reviewed: only implicit `Targeted` resolves the catalog default of four workers, capped by logical processors; explicit values and other modes retain the public default of three. Requested/explicit/effective provenance is preserved through both wrappers and the shard runner, and all 14 stale STAB-06E ordering weights were refreshed without changing the 17-test owner set, serial classification, or hard limit. | Focused owners passed 31/31, 36/36, and 8/8; registration pending. | Not published; user hold. | Not installed. | The measured STAB-06E schedule exceeds 1,200 seconds with three workers but retains more than 200 seconds modeled reserve with four. Peak Targeted memory use may increase on machines with at least four logical processors; no dynamic memory planner was added. |
