...
| STAB-06 | P2: ledger, refs, worktrees, and metrics need bounded hygiene. | Batches A and B are implemented. Batch B adds a strictly read-only disposition report with exact ownership, authoritative bounded remote proof, snapshot-race protection, and fail-closed process handling; it never deletes or prunes. Batches C-D and mutation-capable cleanup remain queued. | Batches A and B registered; batch B head `fc5c154` passed Targeted 109/109. | Not published; user hold. | Not installed. | Batch B has local owner proof only. Candidate worktrees remain `keep` until a complete process-free proof exists; no deletion is enabled. |
