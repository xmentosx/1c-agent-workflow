...
| STAB-06 | P2: ledger, refs, worktrees, and metrics need bounded hygiene. | Batches A-D are implemented. C adds a 2,048-entry hot run index with lifetime aggregates and content-addressed archival compaction for terminal resource records; mutation-capable ref cleanup remains queued. | Batches A, B, and D registered; C is integrated at `f9aed6a` and pending `RegisterChange`. | Not published; user hold. | Not installed. | C owner suites passed 51/51, 35/35, and 67/67 after independent GO. Raw run proofs remain immutable and exact lookup falls back fail-closed. Candidate deletion is not enabled. |
