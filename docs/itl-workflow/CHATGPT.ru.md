...
13. Не завершай конечную задачу после промежуточного успешного шага, проверки, диагностики, исправления или recovery-действия. После каждого tool-вызова самостоятельно оцени, достигнуто ли конечное состояние задачи; пока оно не достигнуто, продолжай работу в этом же ответе без запроса подтверждения пользователя. Для ITL-задач промежуточные `status`, `validate`, `form-validate`, `validate-test-classification`, отдельный успешный тест или исправление очередной ошибки не являются завершением задачи. Завершай работу только после уже требуемого текущим workflow lifecycle/completion gate (включая `fresh passed` там, где он обязателен) либо при реальном blocker, который требует решения, разрешения или внешнего действия пользователя. Длительность работы, объём контекста, ограничение длины ответа и количество tool-вызовов сами по себе не являются основанием для остановки.
```
...
14. Не завершай конечную задачу после промежуточного успешного шага, проверки, диагностики, исправления или recovery-действия. После каждого tool-вызова самостоятельно оцени, достигнуто ли конечное состояние задачи; пока оно не достигнуто, продолжай работу в этом же ответе без запроса подтверждения пользователя. Для ITL-задач промежуточные `status`, `validate`, `form-validate`, `validate-test-classification`, отдельный успешный тест или исправление очередной ошибки не являются завершением задачи. Завершай работу только после уже требуемого текущим workflow lifecycle/completion gate (включая `fresh passed` там, где он обязателен) либо при реальном blocker, который требует решения, разрешения или внешнего действия пользователя. Длительность работы, объём контекста, ограничение длины ответа и количество tool-вызовов сами по себе не являются основанием для остановки.
```
...
10. Не завершай конечную задачу после промежуточного успешного шага, проверки, диагностики, исправления или recovery-действия. После каждого tool-вызова самостоятельно оцени, достигнуто ли конечное состояние задачи; пока оно не достигнуто, продолжай работу в этом же ответе без запроса подтверждения пользователя. Для ITL-задач промежуточные `status`, `validate`, `form-validate`, `validate-test-classification`, отдельный успешный тест или исправление очередной ошибки не являются завершением задачи. Завершай работу только после уже требуемого текущим workflow lifecycle/completion gate (включая `fresh passed` там, где он обязателен) либо при реальном blocker, который требует решения, разрешения или внешнего действия пользователя. Длительность работы, объём контекста, ограничение длины ответа и количество tool-вызовов сами по себе не являются основанием для остановки.
```
