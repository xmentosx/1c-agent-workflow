Describe "Controlled Vanessa Automation patched artifact <revision>" -ForEach @(@{ revision = 'itl-r8' }, @{ revision = 'itl-r9' }, @{ revision = 'itl-r10' }, @{ revision = 'itl-r11' }, @{ revision = 'itl-r12' }) {
    BeforeAll {
    ...
    It "pins the exact upstream source, minimal patch, revision, and toolchain" {
        ...
        $manifest.downstreamRevision | Should -Be $revision
        ...
        @($manifest.patch.expectedChangedPaths) | Should -HaveCount $(if ($revision -in @('itl-r11', 'itl-r12')) { 4 } else { 2 })
        ...
        @($manifest.patch.retainedDownstreamFixes) | Should -HaveCount $(if ($revision -eq 'itl-r12') { 9 } elseif ($revision -eq 'itl-r11') { 7 } elseif ($revision -eq 'itl-r10') { 6 } elseif ($revision -eq 'itl-r9') { 5 } else { 4 })
        if ($revision -in @('itl-r11', 'itl-r12')) {
            $manifest.pairedExtension.required | Should -BeTrue
            $manifest.pairedExtension.protocol | Should -Be 'itl-file-code-v1'
            $manifest.pairedExtension.fileName | Should -Be "VAExtension.1.29-$revision.cfe"
            $manifest.patch.expectedChangedPaths[2] | Should -Match '^features/Libraries/VAExtension/VAExtension/Forms/'
            $manifest.patch.expectedChangedPaths[3] | Should -Be 'lib/VAExtension/Ext/ManagedApplicationModule.bsl'
        }
    ...
    It "builds fail closed through the official upstream flow" {
        ...
        $buildScriptText | Should -Match ([regex]::Escape('[ValidateSet("itl-r4", "itl-r5", "itl-r6", "itl-r7", "itl-r8", "itl-r9", "itl-r10", "itl-r11", "itl-r12")]'))
