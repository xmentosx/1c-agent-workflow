...
Describe "ITL on-demand MCP facade" {
    ...
    It "pins compatible catalogs and keeps development-worktree endpoint assets isolated from main" {
        ...
        $manifest.facadeVersion | Should -Be "0.4.11"
        $manifest.minimumFacadeVersion | Should -Be "0.4.11"
        ...
        $mainSource | Should -Match 'const version = "0\.4\.11"'
    ...
    It "rejects a facade lock mismatch and never falls back to an older installed version" {
        ...
        try {
            ...
            $lock.dependencies.itlOndemandMcp.version = "0.4.11"
            Set-Content -LiteralPath (Join-Path $tempRoot ".agent-1c\dependency-lock.json") -Encoding UTF8 -Value ($lock | ConvertTo-Json -Depth 20)
            $resolved = & {
                . $HelperPath -ProjectRoot $tempRoot -Action help *> $null
                Get-ItlOnDemandMcpExecutablePath -AllowMissing
            }
            $resolved | Should -Be (Join-Path $installRoot "0.4.11\itl-ondemand-mcp-windows-amd64.exe")
    ...
    It "reports a facade contract mismatch as a status warning without throwing" {
        $output = & {
            function Get-ItlOnDemandMcpExecutablePath { throw "ITL_ONDEMAND_FACADE_LOCK_MISMATCH: compatibility='0.4.11' dependencyLock='0.4.8'." }
    ...
    It "installs from the cached release asset when the workflow is an installed copy without Git metadata" {
        ...
        try {
            ...
            $result.path | Should -Be (Join-Path $tempRoot "localapp\ondemand\0.4.11\itl-ondemand-mcp-windows-amd64.exe")
    ...
    It "does not replace a verified cached facade with a stale source-repository build" {
        ...
        try {
            ...
            $result = & {
                ...
                $targetPath = Join-Path $installRoot "0.4.11\itl-ondemand-mcp-windows-amd64.exe"
...
Describe 'On-demand complete database admission plan' {
    BeforeAll {
        ...
        . (Join-Path $repo '.agents/skills/1c-workflow/scripts/lib/agent-1c.roctup-mcp.ps1')
        . (Join-Path $repo '.agents/skills/1c-workflow/scripts/lib/agent-1c.auxiliary.ps1')
