...
Describe "ITL on-demand MCP facade" {
    ...
    It "pins compatible catalogs and keeps development-worktree endpoint assets isolated from main" {
        ...
        $manifest.facadeVersion | Should -Be "0.4.10"
        $manifest.minimumFacadeVersion | Should -Be "0.4.10"
        ...
        $mainSource | Should -Match 'const version = "0\.4\.10"'
        ...
        $manifest.families.'vanessa-ui'.vanessaAutomationArtifact.archiveSha256 | Should -Be "0a5eab8fbc93bf6db053ebcbf99b511c440c737a6f5f6d68840002ac9743bb50"
        $manifest.families.'vanessa-ui'.vanessaAutomationArtifact.epfSha256 | Should -Be "178995192dd02a701fb6301a497c50028cac1b40c40e703de544647c2f84ad5b"
        $manifest.families.'vanessa-ui'.vanessaAutomationArtifact.manifestSha256 | Should -Be "43366ce71aa2657b4d20e46fff6e9a2d6848df0d02d6d63ca8718c6e4a7da781"
        $manifest.families.'vanessa-ui'.vanessaAutomationArtifact.patchSha256 | Should -Be "b12426595cd0fa603698d9682d48bcb00b403ffce747ffdf02e00fd9f7ad5943"
        ...
        [string]$lock.dependencies.itlOndemandMcp.version | Should -Be "0.4.10"
        [string]$lock.dependencies.itlOndemandMcp.releaseTag | Should -Be "itl-ondemand-mcp-v0.4.10"
        [string]$lock.dependencies.itlOndemandMcp.url | Should -Be "https://github.com/xmentosx/1c-agent-workflow/releases/download/itl-ondemand-mcp-v0.4.10/itl-ondemand-mcp-windows-amd64.exe"
        [string]$lock.dependencies.itlOndemandMcp.sha256 | Should -Be "c86185fd94cecf9a39661b6c4358038416480beaf8f465a6bbcef573e97ae998"
    ...
    It "rejects a facade lock mismatch and never falls back to an older installed version" {
        ...
        try {
            ...
            $lock.dependencies.itlOndemandMcp.version = "0.4.10"
            Set-Content -LiteralPath (Join-Path $tempRoot ".agent-1c\dependency-lock.json") -Encoding UTF8 -Value ($lock | ConvertTo-Json -Depth 20)
            $resolved = & {
                . $HelperPath -ProjectRoot $tempRoot -Action help *> $null
                Get-ItlOnDemandMcpExecutablePath -AllowMissing
            }
            $resolved | Should -Be (Join-Path $installRoot "0.4.10\itl-ondemand-mcp-windows-amd64.exe")
    ...
    It "reports a facade contract mismatch as a status warning without throwing" {
        $output = & {
            function Get-ItlOnDemandMcpExecutablePath { throw "ITL_ONDEMAND_FACADE_LOCK_MISMATCH: compatibility='0.4.10' dependencyLock='0.4.8'." }
    ...
    It "installs from the cached release asset when the workflow is an installed copy without Git metadata" {
        ...
        try {
            ...
            $result.path | Should -Be (Join-Path $tempRoot "localapp\ondemand\0.4.10\itl-ondemand-mcp-windows-amd64.exe")
    ...
    It "does not replace a verified cached facade with a stale source-repository build" {
        ...
        try {
            ...
            $result = & {
                ...
                $targetPath = Join-Path $installRoot "0.4.10\itl-ondemand-mcp-windows-amd64.exe"
