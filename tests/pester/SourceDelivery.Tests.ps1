...
Describe "Source develop queue and delivery" {
    ...
    It "fails fast below Windows 10 or Windows Server 2016 before using atomic job-list launch" {
        ...
        $deliveryAst = [Management.Automation.Language.Parser]::ParseFile($DeliveryScript, [ref]$tokens, [ref]$parseErrors)
        @($parseErrors) | Should -BeNullOrEmpty
        $startDefinition = $deliveryAst.FindAll({ param($node) $node -is [Management.Automation.Language.FunctionDefinitionAst] -and $node.Name -eq 'Start-DeliveryProcess' }, $true) | Select-Object -First 1
...
'@
            ...
            $deliveryAst = [Management.Automation.Language.Parser]::ParseFile($DeliveryScript, [ref]$tokens, [ref]$parseErrors)
            @($parseErrors) | Should -BeNullOrEmpty
            $functions = @($deliveryAst.FindAll({
                param($node)
                $node -is [Management.Automation.Language.FunctionDefinitionAst] -and $node.Name -in @('Stop-DeliveryProcessTree', 'Start-DeliveryProcess', 'Close-DeliveryProcessJob', 'Invoke-SourceGate', 'Invoke-ComponentPublicationFinalizer')
            }, $true))
...
'@
            ...
            $functions = @($deliveryAst.FindAll({ param($node) $node -is [Management.Automation.Language.FunctionDefinitionAst] }, $true) | Where-Object Name -in $names)
