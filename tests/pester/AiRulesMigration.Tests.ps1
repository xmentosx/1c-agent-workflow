BeforeAll {
    ...
    $script:TargetAiRulesRef = "itl-main-410951e7-r29"
    $script:TargetAiRulesCommit = "334a5ac6caeececc0a83d214c9f5ba6c98603190"
    $script:TargetAiRulesRevision = 29
...
Describe "ai_rules_1c migration planning" {
    ...
    It "plans monotonic r11 through r28 to r29 migration for all ten clients" {
        $releases = @(
        ...
        foreach ($release in $releases) {
            foreach ($client in $clients) {
                $tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("itl-ai-r29-$($release.revision)-$($client.Replace('-', '_'))-" + [guid]::NewGuid().ToString("N"))
                try {
