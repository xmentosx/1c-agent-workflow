...
Describe "ai_rules_1c migration planning" {
    ...
    It "plans monotonic r11 through r32 to r33 migration for all ten clients" {
        $releases = @(
        ...
        foreach ($release in $releases) {
            foreach ($client in $clients) {
                $tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("itl-ai-r33-$($release.revision)-$($client.Replace('-', '_'))-" + [guid]::NewGuid().ToString("N"))
                try {
