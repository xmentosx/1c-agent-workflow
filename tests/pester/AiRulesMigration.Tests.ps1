    ...
    function New-AiRulesMigrationFixture {
        ...
        $targetConfig = [ordered]@{
            aiRules = [ordered]@{
                ...
                ref = $(if ($ConfigureTarget) { "itl-main-5f3d3f0-r20" } else { "" })
                tools = @($CurrentTool)
            }
        }
        $targetEntry = [ordered]@{
            repo = "https://github.com/xmentosx/itl_ai_rules_1c.git"
            ref = "itl-main-5f3d3f0-r20"
            commit = "151aa980b5e99b3d129e974925e734d9ef0afa3e"
            upstreamRepo = "https://github.com/comol/ai_rules_1c.git"
            upstreamRef = "refs/heads/main"
            upstreamCommit = "5f3d3f03b778d7de38cf2cfb18a20cf3e7ed79d8"
            downstreamRevision = 20
...
Describe "ai_rules_1c migration planning" {
    ...
    It "plans a controlled fork r4 to r19 migration by downstream revision and upstream provenance" {
        ...
        try {
            ...
            $plan.target.downstreamRevision | Should -Be 20
        } finally {
            Remove-Item -LiteralPath $tempRoot -Recurse -Force -ErrorAction SilentlyContinue
        }
    }
    ...
    It "plans the supported r11 to r19 migration" {
        ...
        try {
            ...
            $plan.target.downstreamRevision | Should -Be 20
            $plan.target.ref | Should -Be "itl-main-5f3d3f0-r20"
        } finally {
            Remove-Item -LiteralPath $tempRoot -Recurse -Force -ErrorAction SilentlyContinue
        }
    }
    ...
    It "plans r12 to r19 for every supported single-client installation" {
        ...
        foreach ($client in $clients) {
            ...
            try {
                ...
                $plan.target.downstreamRevision | Should -Be 20 -Because $client
                $plan.target.ref | Should -Be "itl-main-5f3d3f0-r20" -Because $client
                $plan.target.commit | Should -Be "151aa980b5e99b3d129e974925e734d9ef0afa3e" -Because $client
            } finally {
                Remove-Item -LiteralPath $tempRoot -Recurse -Force -ErrorAction SilentlyContinue
            }
        }
    ...
    It "treats the controlled target as current only when ref commit and revision all match" {
        ...
        try {
            New-AiRulesMigrationFixture -Root $tempRoot `
                -CurrentRepo "https://github.com/xmentosx/itl_ai_rules_1c.git" `
                -CurrentRef "itl-main-5f3d3f0-r20" `
                -CurrentCommit "151aa980b5e99b3d129e974925e734d9ef0afa3e" `
                -CurrentUpstreamCommit "5f3d3f03b778d7de38cf2cfb18a20cf3e7ed79d8" `
                -CurrentDownstreamRevision 20
            $plan = & { . $HelperPath -ProjectRoot $tempRoot -Action help *> $null; Get-AiRulesMigrationPlan }
    ...
    It "plans monotonic r11 through r19 to r20 migration for all ten clients" {
        $releases = @(
        ...
        foreach ($release in $releases) {
            foreach ($client in $clients) {
                $tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("itl-ai-r20-$($release.revision)-$($client.Replace('-', '_'))-" + [guid]::NewGuid().ToString("N"))
                try {
                    ...
                    $plan.target.downstreamRevision | Should -Be 20 -Because "r$($release.revision) $client"
                    $plan.target.ref | Should -Be "itl-main-5f3d3f0-r20" -Because "r$($release.revision) $client"
                } finally {
                    Remove-Item -LiteralPath $tempRoot -Recurse -Force -ErrorAction SilentlyContinue
                }
            }
...
Describe "ai_rules_1c transactional migration" {
    It "leaves custom repositories untouched and writes a recovery report" {
        ...
        try {
            ...
            $report.target.ref | Should -Be "itl-main-5f3d3f0-r20"
        } finally {
            Remove-Item -LiteralPath $tempRoot -Recurse -Force -ErrorAction SilentlyContinue
        }
    }
    ...
    It "writes fork config and provenance after an eligible migration" {
        ...
        try {
            ...
            $config.aiRules.ref | Should -Be "itl-main-5f3d3f0-r20"
            $lock.dependencies.aiRules1c.commit | Should -Be "151aa980b5e99b3d129e974925e734d9ef0afa3e"
