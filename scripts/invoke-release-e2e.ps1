            ...
            if ($onDemandMcpTestFixture) {
                ...
                foreach ($spec in @(
                    ...
                    if ([bool]$spec.vanessaSmoke) {
                        ...
                        $expectedAuthoringCalls = @("open_feature_file:file", "check_syntax:file", "load_features:directory")
                        if (($actualAuthoringCalls -join ",") -cne ($expectedAuthoringCalls -join ",")) {
                            throw "Vanessa file authoring smoke did not prove ordinary file authoring and directory loading in the expected order."
                        }
