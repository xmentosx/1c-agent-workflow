# Vanessa Automation 1.2.043.28-itl-r5
...
active. For `reloadAndRunFromLine`, the authoring chain has already qualified
and loaded the feature, so the patch selects the requested scenario from the
current Vanessa Editor content and tree instead of starting a second client-side
file reload. Opening a missing editor tab preserves the original call parameters
and returns to the callback instead of falling through into a duplicate
execution. Its progress-state builder derives the feature directory from the
...
The marker is read fail-closed: missing or non-Boolean values select the
original non-MCP branch. This keeps full-suite tree construction compatible
with parameter structures that predate the MCP marker.
The active editor tab, saved-file, and `.feature` checks remain in place,
and non-MCP calls retain the original file operations.

For ordinary full-suite execution, Vanessa's port availability probe excludes
the client row that is currently being launched. Other configured clients and
ports reported by the operating system remain occupied. This prevents the
requested branch-owned TestClient port from being mistaken for a collision
with its own configuration row and silently replaced with port `48000`.
...
Revision `itl-r5` additionally backports two tool-metadata corrections from
upstream without importing the newer MCP runtime contract. The
`get_data_from_knowledge_base.search_string` input schema is a string (upstream
`b02d884e2636cc4ba6d351861368df14e4bf293b`), and the
`get_window_screenshot_os` description directs callers to
`get_window_list_os` (upstream
`91b1d07584ef2df5858e44c98ff33638bef7b6cf`). Tool names, TestClient ownership,
VanessaExt, and every other `1.2.043.28-itl-r4` behavior remain unchanged.
