...
- Для `codechecker` setup собирает локальный overlay поверх закреплённого upstream image digest. Overlay
  ...
  Существующий `check_1c_code` сохраняет прежний контракт и комбинированную проверку. Отдельный
  `check_1c_logic(code)` выполняет только prompt-анализ логики, потенциальных ошибок и
  производительности: он не вызывает Syntax MCP, не проверяет стиль и не принимает подтверждение
  результата внешней синтаксической проверки.
  Overlay пишет content-safe JSON-телеметрию `itl.codechecker.telemetry.v1`: начало и итог
  `check_1c_code` / `check_1c_logic` / `review_1c_code`, direct/prompt этапы и каждую upstream-попытку. В лог не попадают
...
- `toolsListProxy`: прозрачное сокращение `tools/list` для всех постоянно размещённых MCP;
  ...
  Readiness-проверка ограничена 30 секундами, а обычная пересылка запросов имеет отдельный
  300-секундный таймаут. Клиентские конфиги получают тот же предел для tool-вызовов, поэтому
  долгий штатный поиск или review не превращается в ложную недоступность MCP.
  ...
  До сборки и замены proxy оно проверяет чистоту и актуальность host checkout, а также выполняет
  неинтерактивный `git push --dry-run` registry. Ошибка source drift или credentials останавливает
  действие до начала контейнерной транзакции.
...
`/ready` и сверяет ожидаемые `serverId` и upstream-порт. Готовность должна подтвердиться дважды
подряд в пределах 60 секунд; это закрывает окно восстановления Docker Desktop, когда опубликованный
порт временно ведёт в чужой контейнер. Reconcile пересоздаёт только отсутствующие, неготовые или
неверно маршрутизированные proxy и публикует registry после квалификации. При первой ошибке
квалификации перезапускается только соответствующий direct runtime и выполняется одна повторная
попытка. Отсутствующий direct container считается
...
на каждом интервале. Любой оставшийся `degraded` endpoint делает запуск watchdog неуспешным;
registry при этом обновляется из фактического identity-aware состояния, а не сохраняет ложный
`qualified` от предыдущего запуска.

Watchdog проверяет Windows Docker Engine напрямую через named-pipe `/_ping`, не загружая плагины
Docker CLI. При таймауте native-команды завершается всё дерево дочерних процессов. Если Engine
недоступен, watchdog ограниченно вызывает Docker Desktop CLI: читает `status`, выполняет `start`
для остановленного Desktop или `restart` для недоступного backend и ожидает восстановления Engine.
Таймаут probe при статусе Desktop `running` повторяется без destructive restart. Если daemon
исчезает уже после начальной проверки во время любой
...
одновременно исправляется для следующего штатного создания контейнера. Malformed resource keys
нормализуются, а `docker compose config --quiet` выполняется до любого `down`.

Transport status в registry хранится отдельно от ограниченной read-only MCP-квалификации. В том
числе mismatch embedding-модели Graph публикуется и показывается watchdog как `degraded`; автоматический
reset/reindex базы при этом не выполняется.
...
Пересобрать `Report.txt`, пересоздать включенные MCP-серверы, которые поддерживают `RESET_DATABASE`, и передать им `RESET_DATABASE=true`:
