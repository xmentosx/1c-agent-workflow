...
- ROCTUP MCP Toolkit. Install/cache the OS-specific `MCP_Toolkit*.epf` automatically during init/update under ignored `.agent-1c/tools/roctup-mcp-toolkit`. New `itldev/*` branches auto-start ROCTUP embedded MCP by default without web publication and write branch-local Codex/Kilo MCP client entries. Helper-managed local ports are reserved through the ITL port registry (`ITL_PORT_REGISTRY_SCOPE`, `ITL_PORT_REGISTRY_HOME`) so projects, worktrees, and terminal-server users do not collide. Failures are non-blocking unless `ROCTUP_MCP_REQUIRED=true`.
