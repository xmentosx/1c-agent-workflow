...
`install-roctup-mcp` and `update-roctup-mcp` manage the ignored ROCTUP EPF/skills cache. `start-roctup-mcp` runs `MCP_Toolkit.epf` inside the current branch infobase in embedded mode and writes branch-local config only for the active client. ROCTUP is the preferred branch data MCP and does not require web publication.
