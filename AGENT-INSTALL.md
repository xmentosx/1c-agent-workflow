...
After every successful update, the helper creates one allowlisted local commit in `master` and verifies that tracked state is clean; it never pushes. A no-op update creates no commit. Active `itldev/*` worktrees do not update automatically; run `/itl-refresh` or `/itl-refresh-lite` in each one (or `/itl-refresh-all` from clean `master`) so it receives the updated workflow, binds its own lock-compatible immutable artifacts, and refreshes facade config. Existing legacy files under master `.agent-1c/tools` are left in place for branches not yet refreshed. A facade install/upgrade needs one client reload; later backend starts do not.
