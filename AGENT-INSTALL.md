...
`install-vanessa-automation` downloads only the immutable workflow-pinned `vanessa-automation-single.1.2.043.28-itl-r4.zip`, verifies both archive and EPF SHA-256, atomically installs it under `.agent-1c/tools/vanessa-automation`, creates `tests/features` and `build/test-results/vanessa`, and saves compatibility version plus downstream revision to `.dev.env`. The current lock records `publicationStatus=published`; its immutable URL is live and the canonical archive and EPF SHA checks remain mandatory. Maintainer qualification may still set `ITL_VANESSA_AUTOMATION_SOURCE_BUILD_ARCHIVE` to the exact local candidate. The helper also uses `VANESSA_TEST_PORT_RANGE=48051..48150` by default for final verify and stores the assigned TestClient port in branch state.
