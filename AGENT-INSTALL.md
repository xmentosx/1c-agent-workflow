...
- ITL MCP. Do not ask developers to choose ports, models, or keys during initialization. MCP setup is lazy through `/itl-mcp` after rules are installed. The helper clones or fast-forwards the private GitLab distribution from `ITL_MCP_DISTRIBUTION_REPO` (default `http://gitlabserv01.itland.local/root/MCP-vibecoding1c.git`) into `%LOCALAPPDATA%\ITL\MCP\vibecoding1c\distribution`, rotates license keys into `%LOCALAPPDATA%\ITL\MCP\vibecoding1c`, allocates ports from the local registry, writes ignored Codex/Kilo config for the current scope, and keeps project/branch MCP out of neighboring worktrees. Use `ITL_MCP_DISTRIBUTION_PATH` only as an explicit manual checkout override.
...
`mcp-setup` is the team MCP action exposed as `/itl-mcp`. It clones or fast-forwards the private GitLab MCP distribution, rotates private distribution keys locally, ensures the embedding provider, starts global/project/current-branch MCP servers, and writes Codex/Kilo MCP config. Use `mcp-status` for read-only inspection without clone/pull, `mcp-update` for distribution/key/image refresh, and `mcp-write-client-config` to rewrite only the managed client config blocks.
