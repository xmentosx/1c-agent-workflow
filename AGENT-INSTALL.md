...
`install-vanessa-automation` downloads only the immutable workflow-pinned `vanessa-automation-single.1.2.043.28-itl-r1.zip`, verifies both archive and EPF SHA-256, atomically installs it under `.agent-1c/tools/vanessa-automation`, creates `tests/features` and `build/test-results/vanessa`, and saves compatibility version plus downstream revision to `.dev.env`. The URL in the current lock is an immutable publication contract; while `publicationStatus=contract-only`, it is not claimed to exist and normal download fails closed. Maintainer qualification may set `ITL_VANESSA_AUTOMATION_SOURCE_BUILD_ARCHIVE` to the exact local candidate; the same canonical SHA checks remain mandatory. The helper also uses `VANESSA_TEST_PORT_RANGE=48051..48150` by default for final verify and stores the assigned TestClient port in branch state.
