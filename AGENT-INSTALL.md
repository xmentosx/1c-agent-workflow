...
This package supports Codex, Kilo Code, Claude Code, Cursor, OpenCode, Kimi Code, Qwen Code, Command Code, Cline, and Pi, with exactly one active client per project. The initialization wizard detects the agent client executing the bootstrap, lists it first as the recommended option, and selects it when the answer is left blank. If the execution context is not recognizable, it falls back to Kilo Code; `other` and multi-client installs are not supported.
...
- Local MCP/client runtime state is ignored. Init/update/refresh write it for the active client; `sync-client-mcp -Client` writes the same managed vibecoding1c, on-demand, and UI MCP families to another client's config without changing the active client, rules, or skills. Kilo uses `.kilo/kilo.json`; a neighboring `.kilo/kilo.jsonc` collision blocks writes. Tracked Cursor/OpenCode config requires explicit migration.
...
If the current agent knows its exact head-model family, add `-AgentModel opus5|sonnet5|fable5|gpt56`. Otherwise omit it: the wizard does not ask a model question, leaves `AGENT_MODEL` empty (`auto`), and never infers a model from `-AgentTarget`. JSON init accepts the same optional `agentModel` field; later changes belong to `/rulesmodel`.
...
After successful initialization with a non-empty `userReport`, the final response must be exactly that report rendered as Markdown, with no introduction or conclusion. Do not translate it, convert it to a table, rename or merge fields, reorder or omit lines, summarize it, or replace it with `requiredAction` or a generic success sentence. The helper already includes every safe setting, MCP/Browser line, instruction, and next action in Russian. Do not wrap the report in a code fence, and do not read or reproduce the full `console.log` on success. For every compact-runner action, `userReportOmitted=true` means the complete report exceeded the transport budget: read only the full absolute `userReportPath`, using the file contents for `userReportSource=file` or only its `userReport` JSON property for `userReportSource=status-json`, and preserve the semantic status returned by the helper. For Codex, the report requires a full application restart so the generated project `.codex/config.toml` and MCP servers are loaded, followed by a new `Local` task. If Kilo Code was already open before initialization, the report tells the developer to run `/reload` once in that existing `master` window.
...
- Current agent target. Detect the client that is executing the bootstrap, offer it first, and mark it recommended; an empty interactive answer selects that client. Detect Codex, Kilo Code, Claude Code, Cursor, and OpenCode at minimum, and use the same process identity contract for the other supported clients when their runtime exposes it. Fall back to `kilocode` only when the execution context is not recognizable. The developer can explicitly choose `codex`, `kilocode`, `claude-code`, `cursor`, `opencode`, `kimi`, `qwen`, `command-code`, `cline`, or `pi`. JSON/configured init must still provide `agentTarget` or an exact-one `aiRules.tools` value.
- Head-agent model profile. Do not ask in the wizard. Accept only `opus5`, `sonnet5`, `fable5`, `gpt56`, or empty; a knowledgeable agent passes the slug, otherwise leave `auto`. The selected AI client is not evidence of its model.
...
- Dependency mode. Do not ask during the normal initialization wizard; set `fresh`. It selects the newest ROCTUP and Vanessa UI versions admitted by the packaged on-demand compatibility manifest, never an arbitrary upstream latest. Keep `locked` as an explicit JSON/configured or post-init maintenance option that uses only complete pinned URL/SHA256 metadata. Vanessa Automation itself is workflow-pinned in both modes: compatibility version `1.2.043.28` and downstream revision `itl-r6` are separate fields, and `fresh` never resolves `Pr-Mex/vanessa-automation/releases/latest`. The fresh mode does not advance the immutable controlled fork: both modes keep `aiRules.ref` pinned and never advance its `main`. The on-demand facade v1 supports Windows x64 only and init fails before client-config mutation on another platform.
- UI tools. Init/update best-effort prepare the exact lock versions of `agent-browser` and Windows-MCP. Failure of Node/npm, uv or network is a warning, not an init failure. Both are direct `stdio` entries owned by `ui-tools`; do not route them through the on-demand facade or port registry, do not create a desktop lock, and do not change Kilo Browser Automation automatically.
- Vanessa Automation verification. It is required for executable development branch tests and provides the EPF used by branch-local Vanessa UI MCP. If `VANESSA_AUTOMATION_EPF` is missing or invalid, the helper installs it automatically inside the same init run; do not ask whether it is needed. Store workflow-managed downloads in the immutable `%LOCALAPPDATA%\ITL\artifacts\<family>\<version>\<sha256>` cache and bind the exact path in `.dev.env`; `ITL_ARTIFACT_CACHE_ROOT` is a maintainer/test override. Standard `/itl-check` uses `StartFeaturePlayer` through `TESTMANAGER -> TESTCLIENT` with branch-local `VANESSA_TEST_PORT` as the TestClient launch/connect port in VAParams, not MCP. That packet launch silently enables native VanessaExt in the TestManager session (`QuietInstallVanessaExt`, `useaddin`); this is distinct from installing `VAExtension` into the TestClient infobase. `verify-dev-branch helper alias` remains a compatibility alias. It also checks the local branch infobase event log against `.agent-1c/event-log-baselines/<branch>.json`; fresh non-baseline `Error` records fail verification. Foreign branch Vanessa processes are warnings by default; set `VANESSA_TEST_FOREIGN_WAIT_MODE=wait` only for conservative serialized local runs.
- ROCTUP MCP Toolkit. Init/update/refresh caches the branch-lock-compatible EPF and `itl-ondemand-mcp` executable, then registers `itl-roctup-data` with compact `resolve_tool`/`call_tool` for the active client. Resolution is local; the first inner call starts a client-owned branch backend but does not defer the normal dependency download. Idle/client exit stops it and releases its registry port.
- vibecoding1c MCP. Do not ask developers to choose ports, models, or keys during initialization. Immediately after the project questionnaire and source-infobase safety confirmation, refresh the endpoint registry and complete every required provider, `configId`, and `hostId` choice before downloads, 1C work, or Git baseline work begin. After the helper reports that all initialization questions are complete, the remaining lifecycle is unattended: late MCP setup must apply the saved selection with prompting disabled and stop with an actionable error instead of waiting for input. Use `vibecoding1c-mcp-select` or `vibecoding1c-mcp-setup -Force` for an explicit later reselect. Remote LAN vibecoding1c MCP is the default and is discovered from `VIBECODING1C_MCP_REGISTRY_REPO` (default `http://gitlabserv01.itland.local/root/MCP-vibecoding1c-registry.git`); config-specific remote vibecoding1c MCP always requires an explicit per-server `configId` selection, even when the registry has one configuration. The `code` and `graph` selections do not inherit `configId` or `hostId` from each other. Developers can override each server to local. Local vibecoding1c MCP still clones or fast-forwards the private GitLab distribution from `VIBECODING1C_MCP_DISTRIBUTION_REPO` (default `http://gitlabserv01.itland.local/root/MCP-vibecoding1c.git`) into `%LOCALAPPDATA%\ITL\MCP\vibecoding1c\distribution`, rotates license keys into `%LOCALAPPDATA%\ITL\MCP\vibecoding1c`, allocates ports from the local registry, writes only managed entries for the single active client, reconciles default upstream `ai_rules_1c` MCP entries only when ready vibecoding1c replacements are available, and keeps project/branch vibecoding1c MCP out of neighboring worktrees. New development worktrees inherit a complete `master` selection and rematerialize ready `remote` and `local + project` endpoints in the worktree context without copying raw state. Use `VIBECODING1C_MCP_REGISTRY_PATH` and `VIBECODING1C_MCP_DISTRIBUTION_PATH` only as explicit manual checkout overrides.
- Vanessa UI MCP. Init/update/refresh caches the compatible `client_mcp.cfe` and `VAExtension*.cfe` by version and SHA256 and registers `itl-vanessa-ui` with compact `resolve_tool`/`call_tool`. Its first inner call restores the packaged qualified empty TestManager infobase template, installs `client_mcp` there and `VAExtension` into the development/TestClient infobase, then uses a loopback-only Designer Agent session to set safe mode to `no` for exactly those extensions and verify both properties before recording installation state. The template's dedicated `itl_vanessa_service` user has unsafe-action protection disabled; the workflow neither edits the user's `conf.cfg` nor changes unsafe-action protection in the development infobase. The client-owned backend starts only after that proof. This remains distinct from `/itl-check`.
...
7. Create `.dev.env` from `templates/dev.env.example` when missing. Fill local paths, secrets, and web publication preference; normal wizard initialization writes `DEPENDENCY_MODE=fresh`. Write it as UTF-8 and ensure `.dev.env` is ignored by Git.
...
- Vanessa Automation: check `VANESSA_AUTOMATION_EPF` or the immutable artifact cache; if missing, diagnostic output can mention `install-vanessa-automation`, while normal init/update/refresh installs it automatically.
...
`install-vanessa-automation` downloads only the immutable workflow-pinned Vanessa archive, verifies both archive and EPF SHA-256, streams only the EPF and root license/provenance files into the short project-local `.tx\v` transaction slot, and atomically installs them under `.agent-1c/tools/va`. It does not expand or copy the archive's deep `features` tree. It creates `tests/features` and `build/test-results/vanessa` and saves compatibility version plus downstream revision to `.dev.env`. The lock contains the final immutable URL and hashes but no mutable publication-status field: maintainer qualification uses `ITL_VANESSA_AUTOMATION_SOURCE_BUILD_ARCHIVE` for those exact bytes, publication uploads those bytes, and no post-qualification source commit is required. The helper also uses `VANESSA_TEST_PORT_RANGE=48051..48150` by default for final verify and stores the assigned TestClient port in branch state.
...
Dedicated vibecoding1c MCP hosts are prepared without the agent through `vibecoding1c-mcp-host/install-vibecoding1c-mcp-host.ps1`. The host can read configuration XML files from a Git `sourceRepo` or a local `sourcePath` and can publish shared BookStack and Mantis MCP endpoints. Local `sourcePath` folders can be refreshed manually with `-Action dump-config`, which updates the source infobase from 1C configuration repository storage and then runs incremental `/DumpConfigToFiles` when `ConfigDumpInfo.xml` exists. The managed `nightly-index-*` actions perform that fresh dump, regenerate `Report.txt`, skip unchanged inputs, run incremental Code and Graph indexing, and publish freshness only after both indexes succeed. The host publishes only `registry.json` endpoint/freshness metadata to GitLab; secrets, local paths, infobase credentials, `ONEC_AI_TOKEN`, BookStack tokens, and `MANTIS_API_TOKEN` stay in ignored `vibecoding1c-mcp-host/host.config.json`, the host-local distribution `config.env`, or local environment.

The current workflow refreshes the ITL workflow package, workflow-pinned Vanessa Automation, and ROCTUP MCP Toolkit in fresh mode. The configured immutable `aiRules.ref` remains pinned and is verified against its dependency-lock commit. Vanessa keeps its canonical immutable URL contract, compatibility version, downstream revision, provenance, archive SHA-256, and EPF SHA-256; runtime discovery never rewrites that entry. The helper records resolved workflow and `ai_rules_1c` commits plus other admitted dependency assets in `.agent-1c/dependency-lock.json`. `DEPENDENCY_MODE=locked` remains available as an explicit advanced setting; the normal wizard does not offer it. In locked mode the helper uses only pinned values and stops when a required pin or hash is missing or mismatched.
...
After every successful update, the helper creates one allowlisted local commit in `master` and verifies that tracked state is clean; it never pushes. A no-op update creates no commit. For the execution-guards-v2 cutover, that new master-owned runner then replaces the managed workflow paths in every active Git worktree, records one narrow local commit per changed branch while preserving unrelated working-tree and index changes, deletes obsolete admission/recovery state, and enables the new generation only after every worktree is updated. The cutover does not run branch-local legacy helpers, inspect old ticket semantics, or validate/repair infobase contents. Repeat the same `update-workflow` after an interruption; completed worktrees are no-ops and incomplete worktrees converge to the same package. Then run `/itl-refresh` or `/itl-refresh-lite` in each development branch (or `/itl-refresh-all` from clean `master`) to bind its own immutable artifacts and refresh facade config. Existing legacy files under master `.agent-1c/tools` are left in place for branches not yet refreshed. A facade install/upgrade needs one client reload; later backend starts do not.
...
`ITL_ROUTINE_MODE` is non-interactive and defaults to `off`. In `off`, every `/itl*` command runs in the main agent. In `auto`, `/itl`, `/itl-status`, `/itl-litemode`, and `/itl-result` remain direct while eligible long commands use `itl-routine` only when `SUBAGENT_MODEL_LIGHT` names an explicit inexpensive model. In `on`, every eligible command except `/itl-result` uses the routine and an explicit `SUBAGENT_MODEL_LIGHT` is required. `/itl-result` always stays in the current-dialog agent because that agent owns the task-level final summary; the isolated routine has no reliable preceding-task context. Empty or unknown values safely resolve to `off`; the helper never lets the routine inherit the parent model.
...
`/itl-result` has one command-specific response-composition exception. On success, render its complete `userReport` verbatim as one uninterrupted Markdown block. If the current-dialog agent already has reliable context from the preceding task, append a short `## Итог задачи` with the implemented change, verification evidence, and remaining risks or incomplete work. Do not call another tool to construct that summary or infer it from the export report, Git, or diagnostics. If reliable task context is unavailable, return only the `userReport`. This exception does not change the exact-only response contract of any other `/itl*` command.
