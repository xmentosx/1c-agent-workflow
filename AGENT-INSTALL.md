...
- Vanessa MCP. Do not configure a shared MCP server during project initialization. New `itldev/*` branches leave Vanessa MCP stopped; explicit `start-vanessa-mcp` installs missing branch-local dependencies if needed, starts it for authoring/debugging, and `stop-vanessa-mcp` closes it after use.
...
`install-vanessa-mcp` is a development-branch action. New branches do not run it automatically. `start-vanessa-mcp` installs missing `client_mcp.cfe` and `VAExtension.*.cfe` into the current branch infobase when needed, then starts Vanessa `runMcp` on a branch-local port. Stop it with `stop-vanessa-mcp` after authoring/debugging; the final verify gate remains packet `StartFeaturePlayer` in `TESTMANAGER -> TESTCLIENT` mode.
