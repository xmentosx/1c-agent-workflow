...
- Vanessa UI MCP. Init/update caches the compatible `client_mcp.cfe` and `VAExtension*.cfe` and registers `itl-vanessa-ui` with compact `resolve_tool`/`call_tool`. Its first inner call installs missing branch CFE tooling, then uses a loopback-only Designer Agent session to set safe mode to `no` for exactly `client_mcp` and `VAExtension` and verify both properties before recording installation state. This does not change unsafe-action protection. The client-owned backend starts only after that proof. This remains distinct from `/itl-check`.
