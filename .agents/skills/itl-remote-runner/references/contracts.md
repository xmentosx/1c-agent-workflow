...
`result.json` contains unprofiled samples, iteration and phase outcomes, summary, raw-profile references, loaded-state evidence, source/data/environment identity, resource evidence, limitations and cleanup errors. New results also contain additive `operationEvidence`: `notRequested` for v1 or normalized per-iteration refs/status for v2. Every valid diagnostic iteration has public `operation-evidence.json` and a detailed `operation-evidence.md`; the main `report.md` links the latter. Local and remote routes use the same execution-host renderer. `context.json` and every `private` subtree are excluded from result transfer. Raw packets are separate from native PFF; no file renaming or synthetic native export is permitted. A profile's partial source mapping must remain visible.
