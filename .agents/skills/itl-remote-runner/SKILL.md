...
- Resolve a named connection with `controller --action list` in the current Windows user's private controller home, then use `--host <name>` to ensure its broker and reuse the same pairing from any project. The explicit `--connection` route remains valid. Resolve the exact target from the project; ask together only for missing pairing, workspace, base and permitted operations. Never treat host access as permission for all its bases.
- Default to pull pairing. For a new host, register the controller half by name, stage one `onboard` launcher in an explicitly trusted transfer folder, ensure the TLS broker and ask the user to start only `Start-Worker.cmd` in the Windows session where work should run. Observe `bootstrap-status.json` and pull probe yourself; never ask the user to run a network test, copy console output, choose a 1C executable in the launcher, or type commands into the worker. Prepare exact 1C targets after host connection when necessary. The worker makes the outbound connection without administrator rights, inbound listener, firewall change, SSH server, service or simulated login. Plain HTTP is allowed only on loopback.
