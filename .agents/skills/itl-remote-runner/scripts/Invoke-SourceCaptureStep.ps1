...
$process=$null
try {
    Invoke-WithOneCSessionAdmissionContext -InfoBaseKind $step.base.kind -InfoBasePath $step.base.path -Purpose 'profile-source-capture' -ScriptBlock {
        try {
            ...
            while (-not $process.WaitForExit(100)) {
                ...
                if ($watch.Elapsed.TotalSeconds -ge [double]$spec.timeoutSeconds) { throw 'SOURCE_CAPTURE_STEP_TIMEOUT' }
            }
