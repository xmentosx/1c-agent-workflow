...
If the helper warns that fresh passed verification is missing, ask the developer for explicit confirmation before continuing. For confirmed unverified close, rerun with `-AllowUnverifiedClose`. If `VERIFICATION_POLICY=block`, do not ask for an override; run `/itl-check` first.
