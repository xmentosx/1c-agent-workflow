...
`branchSeedRoot` stores one ignored latest-only seed per hashed source identity and may point to a shared absolute directory through local `BRANCH_SEED_ROOT`. `SOURCE_EVENT_LOG_LOOKBACK_DAYS` sets the auxiliary source error-signature baseline depth in days (default `7`, any non-negative integer); `0` skips source-log reading while branch-local verification remains enabled. A damaged or incomplete source segment is a warning: the seed retains signatures from readable segments and creation continues. For a server source, `serverBaseCopyScript` must implement schema v2: `-Operation capabilities` returns JSON with `schemaVersion: 2`, `restore-seed`, and `event-log-baseline`; a bounded provider additionally advertises `event-log-baseline-lookback`, accepts `EventLogLookbackDays`, and echoes matching `lookbackDays` in the schema v2 signature JSON. A legacy provider without that optional capability skips the auxiliary baseline with a warning. `-Operation restore-seed` accepts `ProjectRoot`, `DevBranchName`, `SeedArtifactPath`, and `DevBranchInfoBasePath`. Older providers fail closed with `SERVER_SEED_PROVIDER_UPGRADE_REQUIRED` only when required seed restore capabilities are absent.
