...
`branchSeedRoot` stores one ignored latest-only seed per hashed source identity and may point to a shared absolute directory through local `BRANCH_SEED_ROOT`. For a file source, the auxiliary source error-signature baseline follows only the latest `.lgp` by a persistent byte cursor; cold start and recovery read at most `SOURCE_EVENT_LOG_BOOTSTRAP_TAIL_BYTES` (default `1048576`, `0` starts at EOF), and older segments are never opened. `SOURCE_EVENT_LOG_LOOKBACK_DAYS=0` disables source-log reading while branch-local verification remains enabled; positive values remain the server-source provider lookback. A damaged or incomplete latest tail is a warning: the seed retains a usable cached baseline when available and creation continues without a full-scan fallback. For a server source, `serverBaseCopyScript` must implement schema v2: `-Operation capabilities` returns JSON with `schemaVersion: 2`, `restore-seed`, and `event-log-baseline`; a bounded provider additionally advertises `event-log-baseline-lookback`, accepts `EventLogLookbackDays`, and echoes matching `lookbackDays` in the schema v2 signature JSON. A legacy provider without that optional capability skips the auxiliary baseline with a warning. `-Operation restore-seed` accepts `ProjectRoot`, `DevBranchName`, `SeedArtifactPath`, and `DevBranchInfoBasePath`. Older providers fail closed with `SERVER_SEED_PROVIDER_UPGRADE_REQUIRED` only when required seed restore capabilities are absent.
