...
Use `.dev.env` for project-local secrets, passwords, web publication values, local tool paths, `DEPENDENCY_MODE`, `VERIFICATION_POLICY`, and optional overrides. A file source infobase may additionally hold the explicitly user-approved, unencrypted `.itl-source-credentials.json` described below so several projects can reuse the same connection. `DESIGNER_MAX_WORKING_SET_MB` overrides the project-level `designerMaxWorkingSetMb` limit for automated Designer processes in the current worktree; the default is 10240 MB and `0` disables the guard. `DESIGNER_OPERATION_TIMEOUT_SECONDS` bounds every automated Designer operation while it waits for command-specific completion evidence (default 3600 seconds). `DESIGNER_COMPLETION_PROBE_TIMEOUT_SECONDS` bounds one internal completion sub-probe (default 30 seconds); recursive dump inspection runs in a disposable worker so a blocked filesystem call cannot freeze the outer monitor. `DESIGNER_STALL_WARNING_SECONDS` marks an operation `stalled-suspected` after no CPU/log/process progress evidence (default 300 seconds). `DESIGNER_STALL_TIMEOUT_SECONDS` then fails after sustained lack of evidence (default 600 seconds) and stops exact owned Designer PIDs. Neither replaces the independent memory guard or overall hard timeout. `DESIGNER_DUMP_STABILITY_SECONDS` controls how long file or `/Out` evidence must remain unchanged before acceptance (default 5 seconds). `GITHUB_TOKEN` (then `GH_TOKEN`) optionally authenticates GitHub API requests; without a token, a fresh dependency resolve falls back to a compatible lock entry only after GitHub rate limiting. Empty password values mean the password is not set.

`branchSeedRoot` stores one ignored latest-only seed per hashed source identity and may point to a shared absolute directory through local `BRANCH_SEED_ROOT`. `SOURCE_EVENT_LOG_BASELINE_ENABLED` controls the auxiliary source error-signature baseline for both source kinds and defaults to `true`. For a file source, the baseline follows only the latest `.lgp` by a persistent byte cursor; cold start and recovery read at most `SOURCE_EVENT_LOG_BOOTSTRAP_TAIL_BYTES` (default `1048576`, `0` starts at EOF), and older segments are never opened. A damaged or incomplete latest tail is a warning: the seed retains a usable cached baseline when available and creation continues without a full-scan fallback. `SOURCE_SERVER_EVENT_LOG_LOOKBACK_DAYS` sets the positive server-source provider lookback and defaults to `7`; `SOURCE_EVENT_LOG_LOOKBACK_DAYS` is retained only as a deprecated compatibility fallback. For a server source, `serverBaseCopyScript` must implement schema v2: `-Operation capabilities` returns JSON with `schemaVersion: 2`, `restore-seed`, and `event-log-baseline`; a bounded provider additionally advertises `event-log-baseline-lookback`, accepts `EventLogLookbackDays`, and echoes matching `lookbackDays` in the schema v2 signature JSON. Server operations covered by the shared database queue also require `recovery-observe`. That read-only operation accepts `ProjectRoot`, the exact `InfoBasePath`, a caller-generated `ObservationId`, and `Sample`; it returns only schema-1 `observationId`, `infoBase`, `databasePresent`, integer `sessionCount`, and `exclusive`, where `exclusive` is true exactly for zero authoritative server sessions. It must not stop sessions. A legacy provider without `event-log-baseline-lookback` skips only the auxiliary baseline with a warning; a provider without `recovery-observe` stops recoverable server work before 1C starts. `-Operation restore-seed` accepts `ProjectRoot`, `DevBranchName`, `SeedArtifactPath`, and `DevBranchInfoBasePath`. Older providers fail closed with `SERVER_SEED_PROVIDER_UPGRADE_REQUIRED` only when required seed restore capabilities are absent.
...
- Missing Vanessa Automation, ROCTUP MCP Toolkit, and Vanessa UI MCP CFE artifacts are cached automatically during init/update/refresh; do not ask whether they are needed. Workflow-managed binaries live in the immutable user-local cache keyed by family, version, and SHA256, while `.dev.env` binds the active branch to its exact lock-compatible paths. CFE installation into a branch infobase and the UI MCP server itself remain on demand.
...
6. Run tool checks, resolve source infobase unsafe-action protection before the first source-base operation, initialize Git, checkout/create `master`, update the source infobase from storage when configured, and dump configuration files to `src/cf`. The baseline commit also installs `-text` for `src/cf/**` and `src/cfe/**`, so Git preserves platform-emitted bytes instead of applying `core.autocrlf`. Source confirmation stays in the current init window without beep/taskbar flashing; interrupted runs repeat only an unproven protection stage.
...
- init/update/refresh: download the facade to `%LOCALAPPDATA%\ITL\MCP\ondemand\<version>`, place backend artifacts under `%LOCALAPPDATA%\ITL\artifacts\<family>\<version>\<sha256>`, verify SHA256, cache only versions admitted by the branch-local `assets/ondemand-mcp/compatibility.json`, and save exact ROCTUP/Vanessa artifact paths. It does not start 1C or a backend.
...
4. Copy only managed workflow files: `.agents/skills/1c-workflow*`, `.agents/skills/product-docs`, `.agents/skills/itl-roctup-1c-data`, `.agents/skills/itl-vanessa-ui-mcp`, `.agents/skills/itl-remote-runner`, `.agents/skills/itl-remote-agent`, `.agents/skills/itl-performance`, Kilo templates, `docs/itl-workflow/`, `templates/`, `install-agent-1c-workflow.ps1`, and `AGENT-INSTALL.md`. Stage each managed path, then replace the live target; on copy failure restore the exact pre-copy tree so retry is not blocked by updater-owned dirt. Never copy or overwrite the target project's root `README.md`. Remove obsolete root workflow docs only when their hashes match a known managed version; preserve divergent files with a warning.
