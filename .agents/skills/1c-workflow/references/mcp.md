...
Vanessa UI MCP is always branch-local and exposed as the stable logical server `itl-vanessa-ui`. The client sees compact `resolve_tool`, `call_tool`, and `finish_database_access`; its full verified catalog remains internal. Static form structure, handlers, commands, bindings, and direct edits use graph/code MCP and sources instead.
