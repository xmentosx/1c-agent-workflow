...
3. Allocate two distinct ports per facade process through the shared ITL registry: the private MCP manager port and a TestClient port from `VANESSA_MCP_TESTCLIENT_PORT_RANGE`. Starting the manager only reserves the TestClient port; editor-only tools do not start TestClient. Before a TestClient-dependent tool, the facade reuses a proven owned process or checks `VANESSA_TESTCLIENT_LICENSE_CAPACITY`, starts one owned process, proves its port, and then proves the manager connection. A client exit or ten idle minutes stops only that manager and its owned TestClient, then releases both leases.
