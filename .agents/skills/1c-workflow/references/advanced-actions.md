...
Extension helper actions and branch-local MCP actions are advanced/helper commands. Keep `set-dev-branch-extension`, `dump-dev-branch-extension`, ROCTUP MCP actions, and Vanessa MCP actions available through helper actions or natural-language requests, but do not generate them as visible Kilo slash commands. New development branches auto-start ROCTUP and Vanessa MCP by default; after manual start, reload or restart Kilo Code if the server is not visible.

ROCTUP MCP actions (`install-roctup-mcp`, `update-roctup-mcp`, `start-roctup-mcp`, `stop-roctup-mcp`, `roctup-mcp-status`) manage the ignored EPF/skills cache and the branch-local embedded data MCP. ROCTUP is the preferred data channel for branch infobases and does not need web publication.
