...
`update-ai-rules` refreshes upstream `ai_rules_1c` managed files with that installer, records the resolved commit in `.agent-1c/dependency-lock.json`, and reapplies the ITL overlay in `USER-RULES.md`. It does not normally append to `AGENTS.md` when upstream `AGENTS.md` already points to `USER-RULES.md`.
