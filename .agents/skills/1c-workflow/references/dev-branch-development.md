...
Для изменения агент последовательно выбирает:

- execution path **quick-fix** или **full-cycle** по upstream policy;
- planning mode **direct** или **OpenSpec** для full-cycle.
...
`/itl` в ветке разработки показывает не просто список команд, а процессную панель: состояние ветки, рекомендуемый следующий шаг, путь разработки, реальные native OpenSpec entrypoints либо точные natural-запросы и дополнительные helper-действия. В свежей чистой ветке с `verification missing` сначала выбирают upstream execution path `quick-fix` или `full-cycle`; для full-cycle затем выбирают direct либо OpenSpec. `/itl-check` выполняется после появления проверяемых изменений или когда предыдущая проверка stale/failed/unknown.
...
- если выяснится, что задача больше quick-fix, продолжи как direct full-cycle при ясном решении либо предложи OpenSpec, когда нужно формальное согласование.
```
...
Если во время quick-fix обнаружен upstream promotion trigger или нарушены eligibility limits, quick-fix прекращается. Дальше задача выполняется как direct full-cycle, если решение ясно, либо через OpenSpec, если требуется формальное исследование и согласование.
...
Обычно начинайте с propose entrypoint, который показывает `/itl`. Explore нужен только когда перед proposal не хватает понимания текущего кода, границ задачи или возможного подхода. `/opsx-*` ниже — пример native-синтаксиса, а не универсальный контракт.
...
```text
Маленькая локальная ошибка без promotion triggers -> quick-fix.

Понятная правка существующей формы, связанных метаданных, нескольких модулей или risk-bearing пути -> direct full-cycle.

Нужно формально исследовать или согласовать требования, архитектуру либо критерии приемки -> OpenSpec.
