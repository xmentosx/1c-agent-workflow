...
9. Activate branch context in the worktree `.dev.env`, then auto-start ROCTUP MCP and Vanessa MCP when their `*_AUTO_START=true` defaults are enabled. If `master` has a complete vibecoding1c MCP selection, copy that selection into the new worktree and materialize ready `remote` and `local + project` endpoints in the worktree context. Write branch-specific endpoints to branch state, `.dev.env`, `.codex/config.toml`, and `.kilo/kilo.json`.
...
11. Report branch, worktree path, copied infobase path, launcher folder/name, ROCTUP/Vanessa status, and publication URL if any.
