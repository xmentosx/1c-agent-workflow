...
9. Activate branch context in the worktree `.dev.env`, then auto-start ROCTUP MCP and Vanessa MCP when their `*_AUTO_START=true` defaults are enabled. Write branch-specific endpoints to branch state, `.dev.env`, `.codex/config.toml`, and `.kilo/kilo.json`.
