...
Waiting applies to lifecycle writers, shared reset reads and runtime MCP leases. Each admission attempt releases its entire partial set before retrying, so a waiting writer cannot prevent the holder from finishing its own main-worktree read. The default budget is 3600 seconds per acquisition; set `LIFECYCLE_LOCK_TIMEOUT_SECONDS` or project config `lifecycleLockTimeoutSeconds` to 0–86400 seconds (0 means immediate failure). Expiry returns `LIFECYCLE_LOCK_WAIT_TIMEOUT` with owner diagnostics. Access/path failures and invalid continuations fail immediately. Preconditions use current config/state after admission; changed `.dev.env` stops with `LIFECYCLE_INPUT_CHANGED` before action execution and requires repeating the same helper.
