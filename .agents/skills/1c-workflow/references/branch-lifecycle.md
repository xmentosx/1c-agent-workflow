...
`update-dev-branch-base` computes the canonical scoped Git-tree fingerprint (excluding `ConfigDumpInfo.xml`) without changing the user's index. A matching configuration/extension fingerprint skips Designer; if normalization is not passed, only Enterprise is retried. A changed fingerprint uses Git changed/untracked paths for the exact partial `-listFile`, including root `Configuration.xml`, Cyrillic paths, and mixed lists. Before Designer starts, every inventory entry must still exist under the hierarchical dump root; an absent file makes explicit `Partial` fail statically and makes the normal `Auto` flow skip partial startup and use a full load, avoiding an interactive missing-file prompt. The partial Designer command uses `-partial -updateConfigDumpInfo`; `ConfigDumpInfo.xml` itself is never placed in `-listFile`. Explicit full load and the one automatic full fallback also use `-updateConfigDumpInfo`. Therefore every successful real load advances the worktree's synchronization cursor together with its copied infobase. Legacy state uses the old Git decision once to seed the fingerprint. Configuration and extension fingerprints are independent; restore/recreate or extension-root changes invalidate the relevant value.
