# Branch Lifecycle Reference

Use this reference for creating, refreshing, switching, listing, or maintaining development branch worktrees and branch infobase copies.

## Git And Worktree Rules

- `master` is the source worktree. New work uses sibling Git worktrees named `<project-folder>-<safe-branch>` by default, without an intermediate worktrees directory. OpenCode 1.18.3 or newer instead uses its native `worktree` workspace adapter through the managed ITL plugin; Kilo and every other client keep the external sibling-worktree flow. Successful init/update/switch for an OpenCode project persists `OPENCODE_EXPERIMENTAL_WORKSPACES=true` in the Windows user environment and prepares the dependency-lock-pinned `@opencode-ai/plugin` package under ignored `.opencode` runtime state. This preparation requires Node.js 22+ with npm and compensates for Windows Desktop builds that request the nonexistent package version `local`. Restart OpenCode so its server sees the capability flag and registers `itl_create_dev_workspace` before creating a branch.
- Use `-UseCurrentWorktree` only when the developer explicitly asks for the legacy single-folder mode.
- Stop on unexpected dirty tracked Git state before worktree creation, legacy switching, copying bases, dumping config files, or advanced close. Refresh is the exception: it checkpoints staged, unstaged, deleted, and non-ignored untracked changes first. Result export accepts the exact dirty working tree and records working-tree provenance in its manifest.
- Worktree-created `itldev/*` lifecycle commands must run from the branch worktree unless the helper explicitly delegates to the main worktree.
- Existing branch state remains authoritative: worktrees previously created under `<project-folder>-worktrees/<branch>` are resumed in place and are not moved to the flat layout.
- Runtime folders such as `.agent-1c/dev-branches/`, `.agent-1c/branch-seed/`, `.agent-1c/event-log-baselines/`, `.agent-1c/event-log-signature-cache/`, `.agent-1c/infobases/`, `.agent-1c/runs/`, `.agent-1c/mcp/`, `.agent-1c/tools/`, `.codex/config.toml`, and `.kilo/kilo.json*` are local ignored state.

## Lifecycle Operation Lock

Every mutating helper action holds `.agent-1c/locks/lifecycle.lock` in its current worktree and publishes phase/status evidence in the ignored `lifecycle-operation.json`. A second mutating action waits before mutation; the runner and `status` show the waiting action, resource, owner and elapsed time. Ordinary actions in different development worktrees may run concurrently. `refresh-dev-branch`, `lock-config-repository-objects`, `close-dev-branch`, and delegated master actions also hold the main-worktree scope when required. `reset-dev-branch` holds its branch lock, uses short shared main-worktree reads for master/seed capture and managed runtime files, and pins the compatible seed through infobase restore and event-log baseline installation. Different branch resets may overlap; main mutations exclude the shared reads and seed replacement waits for active seed readers. `refresh-dev-branch-lite` holds only its branch lock; `refresh-all-dev-branches` owns master exclusively during synchronization, then takes a shared master read lease while at most two lite-refresh workers own their branch locks. It rechecks the captured master commit after this handover; main writers remain excluded while resets can finish shared reads. Branch creation releases the main lock after its short Git worktree phase and continues the long seed restore under the new worktree lock.

`help`, `status`, `list-dev-branches`, `validate`, `check-tools`, platform/publication detection and MCP status actions do not acquire the lifecycle lock. A fresh helper process after workflow copy or branch merge continues the exact operation ID while its parent owns the locks; forged or stale continuation arguments fail with `LIFECYCLE_OPERATION_CONTINUATION_INVALID`. A leftover JSON record without a held file lock is shown as `orphaned` and does not require manual deletion. There is no force-unlock command.

Waiting applies to lifecycle writers and shared reset reads. Native phases release lifecycle locks before waiting for their exact execution guard, then reacquire and revalidate current config/state before start. The default lifecycle budget is 3600 seconds per acquisition; set `LIFECYCLE_LOCK_TIMEOUT_SECONDS` or project config `lifecycleLockTimeoutSeconds` to 0–86400 seconds (0 means immediate failure). Expiry returns `LIFECYCLE_LOCK_WAIT_TIMEOUT` with owner diagnostics. Guard waits are separately bounded and all-or-none. Changed `.dev.env` stops with `LIFECYCLE_INPUT_CHANGED` before native execution and requires repeating the helper.

To cancel waiting, create an empty file at the exact `cancelPath` printed by `status` or in the waiting run's `stageDetail`. The helper removes its waiter/cancel files, releases its own leases and returns `cancelled` (exit 2); the compact runner preserves that terminal status. Do not delete holder locks or edit lifecycle JSON. Waiting resumes automatically on release; cancellation and timeout do not automatically rerun the operation.

## NEW_DEV_BRANCH / NEW_EXTENSION_DEV_BRANCH

Goal: create an isolated development branch worktree plus isolated copied infobase.

Use the monitored launcher by default when `DEV_BRANCH_UNSAFE_ACTION_PROTECTION_SETUP=manual-confirm` so fallback unsafe-action protection confirmation is visible. A valid master-local source confirmation makes this step question-free. Direct `agent-1c.ps1` branch creation is allowed only when that confirmation is valid or explicit non-interactive automation sets `DEV_BRANCH_UNSAFE_ACTION_PROTECTION_SETUP=skip`.

1. Check the current Git worktree is clean and the command is run from `master`.
2. Sync/checkout `master` in the main worktree and pull `--ff-only` when a remote/upstream exists.
3. Create `itldev/<safe-dev-branch-name>` in a sibling worktree unless explicit branch/worktree paths are supplied. In a new OpenCode project, the managed plugin asks the native workspace adapter to create that exact branch, waits for checkout readiness, and lets the helper adopt the existing worktree without issuing another `git worktree add`.
4. Copy `.dev.env` into the new worktree.
5. Restore the branch infobase from the single compatible latest-only seed. Never fall back to a direct source copy.
   - File base: copy the closed seed `1Cv8.1CD` and its optional `DoNotCopy.txt` marker; raw `1Cv8Log` is never copied.
   - Server base: restore the seed DT through `serverBaseCopyScript` schema v2 capabilities `restore-seed` and `event-log-baseline`; legacy providers fail with `SERVER_SEED_PROVIDER_UPGRADE_REQUIRED`.
   - The `ConfigDumpInfo.xml` committed in `master` is the initial synchronization cursor for this one-time source-base copy. After creation, the development worktree and its copied infobase own that cursor; later master refreshes must not replace it.
6. If `sourceUsesRepository=true`, unbind the development branch copy from 1C configuration repository storage without repository parameters.
7. Resolve unsafe-action protection immediately after repository unbind. Reuse a context-matching source confirmation from the main worktree; otherwise run the visible Russian confirmation/Configurator loop for the copied base. Persist `unsafe-action-protection-resolved` before continuing so resume never recopies the base or repeats a proven answer.
8. Register the branch infobase in `%APPDATA%\1C\1CEStart\ibases.v8i` under `/ITL/<project-root-name>` with entry name `<project-root-name>-<safe-development-branch-name>`, matching the canonical worktree folder name.
9. Save branch state to `.agent-1c/dev-branches/<safe-dev-branch-name>.json` inside the worktree, including `createdWithWorktree`, `worktreePath`, `mainWorktreePath`, launcher metadata, `devBranchKind`, publication status fields, legacy MCP migration fields, and Vanessa Automation verification fields. Provider-marked OpenCode branches are the only exception: their canonical state and non-recoverable runtime live below the main worktree's ignored `.agent-1c`, and their adopted Git worktree is locked after validation.
10. Activate branch context in `.dev.env`, reconcile workflow-managed dependency pins from the branch's newly merged template when `DEPENDENCY_MODE=fresh`, bind exact lock-compatible ROCTUP/Vanessa artifacts from the immutable user-local cache, and register the stable `itl-roctup-data` and `itl-vanessa-ui` stdio facades with compact `resolve_tool`/`call_tool` surfaces for the active client. Refresh fills a missing cache entry before the 1C load; neither backend nor 1C starts merely to populate the cache, and the backend still starts only on the first inner call. `DEPENDENCY_MODE=locked` remains immutable and fails closed when its explicit pins are incompatible with the refreshed workflow. If `master` has a complete vibecoding1c MCP selection, copy and rematerialize it for the new worktree.
11. If web publication is enabled, run the helper-owned publication cycle: automatic publication only when `WEB_PUBLISH_AUTO=true`, otherwise manual URL entry or skip. Best-effort legacy branch Data MCP connects only after a publication URL exists.
12. Install the source error-signature baseline stored beside the seed. The new branch starts with its own clean event log; no raw source log files are transferred.
13. Persist `initializationStatus=enterprise-normalization-pending`, then run Enterprise with the bundled auto-update EPF against the copied branch infobase only. Save `enterpriseNormalizationStatus`, reason, error, time, EPF and log evidence; set `initializationStatus=ready` only after success.
14. Report branch, worktree/base paths, launcher, MCP/publication state, event-log scan evidence, and Enterprise normalization state. A new `Получение обновлений программы` error is reported as a non-blocking warning only when it names `update-api.1c.ru` and explicitly says that Internet-support services are prohibited; every other new event-log error signature remains blocking.
15. For an extension branch, run the separately transactional initialization phase inside the new worktree when `ExtensionInitMode`, `ExtensionName`, and optional CFE path were collected in chat. If they are explicitly unknown, persist `extensionInitializationStatus=pending`; every development action blocks with `EXTENSION_INIT_REQUIRED` until the agent collects the values on first entry and completes initialization. Never hand a PowerShell command to the developer.
16. Print the Russian instruction that the current folder stayed on `master`, report the structured new worktree path, and open a separate window of the selected agent or IDE best-effort. For Codex, tell the developer to add the existing worktree folder as a separate project, restart the Codex app so it loads that project's `.codex/config.toml` and MCP servers, and only then create a `Local` task; do not ask Codex to create another worktree. Other newly opened clients read their project context on startup unless their adapter says otherwise. Any reload required after initial project installation belongs to the already-open `master` client window before branch creation.

If the final Enterprise step fails, preserve the worktree, copied infobase, launcher, and failed state. Repeating `new-dev-branch` reuses those assets and retries normalization without copying the base again. Branches created by older workflow versions have no marker; before the first Enterprise-bound ROCTUP, Vanessa UI, Vanessa Automation, publication, or legacy Data MCP action, normalize them once with reason `legacy-preflight`. Never run this normalization against the source infobase.

## FORK_DEV_BRANCH

Run `fork-dev-branch` from the active ready `itldev/*` worktree when one line of work must split into independent continuations. The helper checkpoints all non-ignored Git changes, fixes that commit as the common branch point, stops source-owned runtime, and writes a resumable ignored snapshot below main `.agent-1c/fork-staging/<target>/`. File bases copy the closed `1Cv8.1CD` plus optional `DoNotCopy.txt`; server bases use a DT. The target gets a sibling worktree, an independent base restored atomically from that snapshot, a new launcher record, stable on-demand MCP facades, and no inherited publication, PID, port, lease, lock, or active process.

Source `1Cv8Log`, its signature boundary, branch state, and referenced verification/Vanessa reports and logs are copied into immutable target `.agent-1c/fork-history/<fork-id>/`. The target event log remains separate; the inherited signatures form its baseline so pre-fork errors are historical rather than new. `fresh passed` is inherited only when the source proof was fresh and the canonical verification fingerprint, dependency/platform environment fingerprint, and restored base snapshot identity all match. Otherwise evidence is still retained and linked, but verification is marked `stale`. New verification-scope edits naturally make inherited proof stale. Repeat the same fork command after interruption; a completed identical fork is idempotent, while a mismatched existing branch or snapshot fails closed.

If branch creation used `DEV_BRANCH_UNSAFE_ACTION_PROTECTION_SETUP=skip` before protection was actually disabled, recover from the branch worktree through the monitored `configure-dev-branch-unsafe-action-protection` helper action. It reopens the normal visible Designer confirmation flow and can record an empty-password local user through `-InfoBaseUser`; do not recreate the branch or mark protection confirmed without the developer's explicit UI action and confirmation.

## SYNC_DEV_BRANCHES

Run `sync-dev-branches -PeerDevBranchName <name>` from either ready development worktree when two branches must exchange their current 1C implementation before either result reaches `master`. Before locking or checkpointing either worktree, the helper requests both databases and their recorded runtime resources atomically from the same database access coordinator. A busy peer causes bounded waiting without partially acquiring the other database or taking lifecycle locks. Both participants and their resources are rechecked after waiting; a changed target, manager or coordinator stops the operation before source changes. Configuration branches synchronize only their configured `src/cf/**` root; extension branches must target the same extension and synchronize only its `src/cfe/<extension>/**` root. Each branch retains its own `ConfigDumpInfo.xml`. OpenSpec, feature files, tests, documentation, workflow files, and every other repository path remain branch-local.

The helper computes a three-way combined source tree without merging either branch into the other's Git history, creates an independent source-only commit in each branch, proves equal cursor-independent fingerprints, and loads the result into both branch infobases. Verification becomes stale in both branches. The resulting common implementation may therefore be checked and exported entirely from either branch. For a source conflict, resolve only the listed source files in the initiating worktree, run `git add`, and repeat the same command there; the helper owns both commits and propagation to the peer.

### Group source synchronization

For more than two branches, use `sync-dev-branches -BranchSyncRequestPath
<absolute-json-path>` from the initiating branch. Store the UTF-8 request under
its ignored `.agent-1c/source-sync/` directory. Example from `itldev/branch1`:

```json
{"schemaVersion":1,"peers":["branch2","branch3"],"recipients":["branch1","branch2","branch3"]}
```

Both lists must be arrays of distinct participating branch names; `recipients`
must include the initiator. To collect all three inputs only into branch1, use
`"recipients":["branch1"]`. This request is mutually exclusive with
`-PeerDevBranchName`. Do not implement a group as successive pair calls: a peer
processed early would not receive later peers' changes.

The helper reserves all participant databases and recorded managers atomically
before any lifecycle lock, validates their exact targets again after waiting,
and rejects unrelated uncommitted paths before checkpointing any participant.
It pins all input heads, composes every source input before propagation, validates
the composed source, and creates each recipient's independent source commit.
Only then does it load the explicit recipients. Each retains its own dump cursor
and non-source tree. Source-only participants receive no source or database load.

The checksummed plan in `.agent-1c/source-sync/<id>.json` records the exact
request hash, pinned inputs, common fingerprint, pending file installation,
recipient commits and database load receipts. Internal Git refs retain these
objects for resumption. Repeat the original request from the original initiator.
Resolve listed conflicts there and stage the resolution with `git add`; do not
create the helper's commits or edit its plan. A late source conflict occurs
before any recipient database load. An interrupted ref/plan write is reconciled
against the planned commit; unrelated staged or working source edits are retained
and reported instead of overwritten.

A completed database load is reused only with a matching group, fingerprint and
HEAD receipt. The coordinator also retains intent/completion receipts separately
for Designer load, Enterprise normalization, runtime refresh, cursor commit and
branch state. A lost local plan write is reconciled from that authority; finished
Designer/Enterprise phases are not repeated. Only the acknowledged cursor change
may be committed during continuation; foreign source/index edits are preserved.
The original coordinator is part of the saved group and cannot change on retry.

After owner failure use `access-recover-workflow` for its ticket. A phase boundary
permits release only after the original producer exits, all reserved databases
are inspected twice as idle/exclusive, and the native journal has no later work
or pending restoration. Then repeat the original group helper. An intent with
no subsequent native work can start afresh; an unacknowledged native effect stays
`DEV_BRANCH_SOURCE_SYNC_LOAD_UNCONFIRMED`, with completed recipients preserved.
Absence of a receipt is not permission to repeat a potentially partly applied
Enterprise update. Recovery of such effects and server inspection remain open.
Queue waiting remains bounded and unrelated databases can continue; do not remove
a live or ambiguous owner's lease by age. The final report includes source and
load status for every participant even when delivery is partial.

Before either checkpoint, the helper also checks both branches for an existing helper-owned lifecycle merge. A pending `refresh-dev-branch` or `refresh-dev-branch-lite` is a recoverable prerequisite: the result identifies the exact branch, worktree, original wrapper, stage, and recorded conflict paths with `errorCategory=merge-conflict`. The agent continues that wrapper, performs progressive semantic repair when required, asks only for an evidence-backed incompatible business choice, and resumes the original source synchronization after refresh success. A different pending lifecycle operation requires an explicit user decision because completing it may invalidate the requested synchronization. An unowned or inconsistent Git operation remains fail-closed; never abort it, edit lifecycle state, or create its commit manually.

For extension branches, ask in chat for Empty or CFE, the extension name, and the CFE path when applicable before launching branch creation. Branch copy and extension initialization remain separate transactional phases, but `new-extension-dev-branch` orchestrates both in one user scenario. If the values are unknown, only the second phase is deferred and persisted as pending.

## Extension Helpers

The composite `new-extension-dev-branch` invokes `init-dev-branch-extension -ExtensionInitMode Empty|Cfe -ExtensionName <name> [-ExtensionSourcePath <file.cfe>]` internally from the new worktree. A pending or failed second phase can be resumed without copying the branch infobase again. `Empty` uses the installed `cfe-init.ps1` scaffold and Designer `/LoadConfigFromFiles ... -Extension`; `Cfe` loads the binary directly with `/LoadCfg ... -Extension`. Neither mode uses Designer Agent, `AgentMode`, `/Extension`, or CFE unpacking. The helper snapshots the copied infobase, rolls back on failure, dumps normalized sources only to `src/cfe/<ExtensionName>`, validates them, and records `ready` only after full success.

A configuration branch may contain several related features. An extension branch may also contain several features or OpenSpec changes, but only for its one selected extension. A second changed CFE requires a separate branch, worktree, and branch infobase; the lifecycle intentionally has no `extensions[]` state. Update, check, dump, result, and close reject changed paths under another `src/cfe/<Name>` with `EXTENSION_BRANCH_SINGLE_ARTIFACT` while tolerating unchanged baseline CFE directories.

`set-dev-branch-extension` only records recovery context for an extension already present in the branch infobase; a missing slot fails with `EXTENSION_RECOVERY_SLOT_MISSING` before state or `.dev.env` changes. `dump-dev-branch-extension` writes a temporary dump, validates its root/name/CFE structure, and replaces the canonical dump transactionally. `/update1cbase` is the subsequent development loop after initialization.

Rules:

- Find branch state from `DevBranchName` or current branch.
- If the state belongs to another worktree, report `worktreePath` and tell the developer to open that folder.
- For legacy branches, require a clean Git worktree and checkout the saved branch.
- For a new extension branch without initialized extension state, ask the developer for Empty or CFE, extension name, and optional CFE path in chat, then invoke `init-dev-branch-extension` internally. Never tell the developer to run it. Use `set-dev-branch-extension` only for a manually created legacy extension.

## Branch Context And Base Update

`activate-dev-branch-context` writes the current branch infobase values into `.dev.env` so upstream `ai_rules_1c` commands such as `/update1cbase`, `/loadfrom1cbase`, and `/getconfigfiles` target the copied branch infobase.

`update-dev-branch-base` computes the canonical scoped Git-tree fingerprint (excluding `ConfigDumpInfo.xml`) without changing the user's index. A matching configuration/extension fingerprint skips Designer; explicit `Full` never skips it. If normalization is not passed, only Enterprise is retried. For a changed fingerprint, the exact partial `-listFile` is the Git tree diff from the last successfully loaded Designer tree to the current effective tree. A missing/garbage-collected prior tree, legacy state, deletion, or unprovable diff selects one safe full load. `ConfigDumpInfo.xml` participates in neither readiness nor partial inventory. The partial Designer command uses `-partial -updateConfigDumpInfo`; explicit full and the one automatic full fallback also use `-updateConfigDumpInfo`. Fingerprint and tree cursor advance only after successful Designer completion; configuration and extension proofs are independent.

Immediately before invoking a potentially mutating Designer load, the helper atomically clears only the matching configuration/extension Designer fingerprint in branch state and records `configLoadStatus=pending`. It restores that fingerprint only after the combined `/LoadConfigFromFiles ... /UpdateDBCfg` process succeeds. Failure, timeout, or interruption therefore leaves the proof invalid; even when the source returns to the previous content and Git has no partial list, the next normal check performs a recovery full load instead of legacy-seeding the fingerprint. Before invoking Designer, the helper also snapshots only the branch-owned `ConfigDumpInfo.xml` outside the source tree. A failed or interrupted partial load restores that cursor before either returning the error or trying full fallback; a failed full load restores the original cursor as well. The infobase itself has no transactional snapshot, so a double failure still records `fallback-failed`, both errors/logs, leaves the last-loaded commit unchanged, skips normalization/MCP restart/verification, and requires recreating the branch copy for safe recovery. A successful fallback records `configLoadStatus=fallback-succeeded` and `lastConfigLoadMode=full-fallback`. Full load remains a real-failure fallback, not a special case for root XML.

After any real partial/full configuration or extension load, normalization is marked pending and ITL launches Enterprise through the bundled auto-update EPF to apply update handlers and answer the legal-copy prompt non-interactively. The EPF records the actual update result or exception in a run-bound JSON file under `.agent-1c/runs/auto-update`; ITL records `passed` only when that file matches the current run, reports success, and the Enterprise process completes. A product handler error is `failed`, with the result path and cause retained in branch state; verification does not start. Older `passed` state without this proof is unverified: a no-op check stops and requires an explicit `update-dev-branch-base` recovery instead of replaying possible update side effects silently. The same exact-dev-infobase drain runs before Designer and before every normalization attempt: it first stops workflow-owned Vanessa/ROCTUP, then closes remaining local sessions with a proven exact `/F` or `/S` target even when user-launched, and clears their stale runtime records. Source, other-branch, ambiguous, and PID-reused processes remain protected. Later tool calls create fresh instances without client reload. The default timeout is 900 seconds; use `DEV_BRANCH_AUTO_UPDATE_TIMEOUT_SECONDS` only for a different positive limit.

For a failed, timed-out, or suspiciously long Enterprise normalization, diagnose the named operation from its start time, helper state, run result JSON, exact owned process progress, and fresh branch `1Cv8Log` entries. An implicit no-op check or publication preflight does not retry a prior failed normalization; first resolve the cause, then explicitly run `update-dev-branch-base`. Treat `/Out` as secondary evidence because Enterprise often leaves it empty. A fresh error correlated with the operation plus no meaningful progress far beyond its expected duration is evidence of failure or a stall; do not wait only for the hard timeout when the cause is already established. Follow helper-owned recovery and never kill arbitrary 1C PIDs.

After every successful or failed Vanessa verification, stop branch-owned `TESTMANAGER`/`TESTCLIENT` processes and fail the verification if cleanup cannot be proved. Use the advanced `stop-dev-branch-test-clients` recovery action for leftovers from older runs; it matches the current branch infobase/worktree and must not stop foreign worktrees.

Development branch changes must never be loaded directly into the source infobase connected to 1C configuration repository storage.

## STATUS / LIST / SWITCH

- `status` shows the current lifecycle operation or orphaned record, worktree, branch, initialization/normalization/config-load evidence, event-log reader/cache/count/duration, verification/MCP summaries, and target worktree paths.
- `list-dev-branches` shows active branches, worktree/base state, verification and vibecoding1c status; general `status`/`doctor` also report the facade executable and owned on-demand instances.
- `switch-master` clears active branch infobase values and returns legacy single-folder checkouts to `master`.
- `switch-dev-branch` is mainly legacy recovery. For worktree-created branches, report the saved worktree path instead of changing the current folder.

## REFRESH_DEV_BRANCH

Goal: refresh the current development branch from fresh `master` and source state.

1. Require an active `itldev/*` worktree. The compact runners for `refresh-dev-branch` and `refresh-dev-branch-lite` first delegate to the clean tracked workflow runtime from the checked-out `master` worktree while retaining the development worktree as `ProjectRoot`; no other branch command uses this bootstrap. The master runtime completes helper-owned merge recovery first, then commits all staged, unstaged, deleted, and non-ignored untracked paths as `chore: checkpoint before branch refresh`; ignored runtime is never staged. Other merge/rebase/cherry-pick/revert state and unmerged paths block before 1C or merge activity.
2. Refresh `master` through the main worktree and reuse a compatible seed. `SOURCE_REPOSITORY_UPDATE_MODE=workflow` first updates the source infobase from storage; `external` captures its current state without running repository or database-configuration updates against source. Explicit `sync-master` always rebuilds the seed so its data and baseline are current.
3. Merge fresh `master` into the branch while retaining every `ConfigDumpInfo.xml` version from the branch head. When this is the branch's first merge of the managed `src/cf/**`/`src/cfe/** -text` contract, ignore end-of-line whitespace for that transition only and rebuild both source indexes before the merge commit; later merges remain strict. Cursor-only conflicts are resolved automatically in favor of the branch; any other merge conflict records the exact lifecycle merge transaction and returns `errorCategory=merge-conflict`. Before its merge commit and before any changed configuration is loaded into Designer, the helper runs `cf-validate`. During a merge it additionally inspects only the risk set—files changed on both sides relative to `merge-base` plus explicit `repairPaths`—for merge-created duplicate BSL declarations and routes XML forms and metadata structures through their installed specialized validators; master-only files do not trigger this per-file work. Automatic overlaps whose contents agree in both parents, the index, and the worktree are excluded: ancestry retained by a reset must not turn an inherited diagnostic into a merge failure. Explicit `repairPaths` and the cross-file UUID check remain unconditional within their existing scope. A failure returns `errorCategory=source-integrity`, writes a structured report with exact `base`/branch/master identities, and keeps the merge resumable. For either category the agent starts with the conflicting semantic unit and preserves both compatible deltas; it widens to callers, related source, history, tests, OpenSpec, and required product documentation only for a concrete uncertainty or failed check. Related staged paths below the managed 1C source roots are recorded as `repairPaths`; unrelated staged, unstaged, or untracked paths remain blocking. The agent runs `git add` and repeats the same command; it asks the user only when authoritative evidence leaves incompatible business outcomes and never creates the merge commit manually. The repeated command validates the operation, branch, original branch HEAD, exact `MERGE_HEAD`, staged paths, and clean worktree state before it runs `git commit --no-edit` and continues post-merge. `refresh-dev-branch-lite` and `close-dev-branch` use the same recovery contract.
4. After the merge, hand the same lifecycle operation and held locks to the branch-local helper from the updated branch HEAD. That helper owns config load, Enterprise normalization, MCP/state updates, and the final report.
5. Regenerate context-specific Kilo wrappers if workflow files changed.
6. Update the branch infobase from changed files.
7. Keep the stable facade config and let the next tool call create fresh backend instances after a real load.
8. Re-check whether verification is fresh; stale or unknown verification must be handled before result export.

If post-merge configuration loading fails after the merge transaction is committed, correct the reported source defect and repeat the original `refresh-dev-branch` or `refresh-dev-branch-lite` command. The helper accepts a clean, linear descendant of the recorded post-merge HEAD only on the recorded branch, after validating the original merge parents and pinned master SHA; a later merge, another operation, a foreign branch, or an unrelated HEAD remains blocked. The repeated command does not merge or select master again: it rebinds dependencies, checks the test inventory, loads configuration, normalizes Enterprise, and reconciles MCP before clearing the pending transaction. A failed stage retains pending for the same-command retry. Successful refresh does not turn a failed `/itl-check` into passed verification, and stale verification cannot authorize result export. The existing fresh full verification recovery remains available after a successful configuration load and Enterprise normalization.

`refresh-dev-branch-lite` executes steps 1 and 3–8 against the exact current `master` SHA. It never updates or reads source and never touches the seed. From a clean managed `master`, `refresh-all-dev-branches` performs one `sync-master`, fixes that SHA, and runs lite refresh for every active ready branch with at most two workers; branch failures are isolated and aggregated.

### Refresh-all recovery

The aggregate's `resultManifestPath` contains its pinned `masterCommit` and each branch's status, actual `refreshMasterCommit`, category, stage, worktree, child status/log paths and required action. A child success counts only when its completed refresh proves the pinned SHA. Missing evidence or completion of an older refresh remains blocked. Final counters include all inventory errors and finished workers; `branch-aggregate` distinguishes partial branch failures from a runner crash.

On `recover-refresh-all:`, continue each recorded action in its named worktree. For semantic conflicts, inspect the conflicting unit, preserve compatible intent (including compatible additions to managed `.gitignore` rules), stage the repaired paths and repeat the same ITL helper. Do not replace whole files with one side, manually commit a lifecycle merge, reset a branch or delete state. Resolve classification requests on successful branches too. For `refresh-target`, inspect the pending transaction, complete its original helper action without `-ExpectedMasterCommit`, then invoke `refresh-dev-branch-lite -ExpectedMasterCommit <aggregate masterCommit>` using the managed master runner from that branch worktree. Keep the aggregate pin: if master moved, report the mismatch rather than silently changing the target or resynchronizing master. Report the original aggregate and subsequent verified per-branch results; do not rewrite the original run artifacts. Ask only when evidence leaves incompatible intent or recovery needs additional user input.

On `LIFECYCLE_MERGE_PRESERVATION_REVIEW_REQUIRED`, follow [merge preservation](merge-preservation.md): inspect the retained two-sided changes, repair compatible intent or record result-bound evidence for a justified replacement, then repeat the same helper. A clear Git conflict list and valid XML/BSL do not prove that both sides' changes survived.

## RESET_DEV_BRANCH / LOCK_CONFIG_REPOSITORY_OBJECTS

`reset-dev-branch` supports ready configuration branches. It checkpoints current work, validates the local-master seed, then writes an ignored archive below `<main>/.agent-1c/branch-archives/<branch>/<UTC>-<head>/`: guarded `infobase.dt`, hashed non-configuration delta in `files/`, and schema-1 `manifest.json` with deleted and excluded configuration paths. Only after verifying the archive does it create a normal cleanup commit whose tree exactly equals the recorded local `master`, restore the file/server branch base from that exact seed, unbind a restored repository-backed copy, and reinitialize branch runtime state. The state journal remains `resetting` until completion; repeat the same command to resume. The final report always prints absolute archive, DT, commit, and infobase paths. Extension branches fail before checkpoint or mutation.

Reset may discard a pending `refresh-dev-branch` or `refresh-dev-branch-lite` at stage `merged` when the recorded merge belongs to this branch and worktree, its exact parents and target are valid, its post-merge HEAD is an ancestor of current HEAD, and no Git merge is active. It first checkpoints tracked and non-ignored untracked work, including `handoffs/`, then verifies DT and file archive, restores the exact local-master seed, and clears every pending merge and refresh field only when reset completes. An interrupted reset resumes from its own journal without replaying refresh. The report identifies the refresh as discarded by reset. An active conflict merge stops before checkpoint because its uncommitted resolution cannot yet be archived safely; reset never aborts that merge. Pending cross-branch source synchronization still requires recovery by its owning synchronization helper.

`lock-config-repository-objects` supports ready configuration branches with configured source storage. It maps committed, staged, unstaged, and non-ignored untracked `src/cf` delta from merge-base with local `master`, fails before Designer on unknown paths, writes the official XML object list into ignored run artifacts, and calls only `/ConfigurationRepositoryLock -Objects` against the source base. An empty list is a successful no-op; there is no full-configuration fallback.

## CLOSE_DEV_BRANCH

`clean-artifacts` lists or removes only manifest-backed CF/CFE pairs, completed run directories, and marked reset archives in the current worktree; on `master` it also considers branch-reset archives. The `.dev.env` policy retains 3 latest result pairs, 2 latest recovery archives and at least 3 completed runs per category, with excess runs also required to be 7 days old. Automatic cleanup is best-effort after successful relevant actions and can be disabled with `ITL_ARTIFACT_CLEANUP_ENABLED=false`; `/itl-clean` remains explicit. Active/reference paths and unknown/unmarked artifacts are retained. `/itl-status` reports verified candidate and eligible bytes, not a whole-disk total.

`delete-dev-branch` runs from `master`, asks once, stops exact branch runtime and releases its port allocations, then removes the local `itldev/<safe-name>` worktree including dirty and ignored content, its Git branch, launcher record, managed file base, owned result files and archives, and an exact automatic web publication. It does not merge, test, require a clean branch, delete remote refs, touch a source base or shared cache. An ignored main-worktree journal lets the same command resume after interruption. Server infobases and manual web publications are reported as leftovers for their owners because ITL does not own a generic deletion provider for them.

`close-dev-branch` is advanced bookkeeping for hiding a branch from active lists. It follows the same verification policy as result export, exclusively waits for active MCP calls, stops all owned branch backends, and records close metadata. Legacy and non-OpenCode worktrees or infobases are never deleted automatically. For a newly provider-marked OpenCode workspace, the managed plugin then warps the session to main, explicitly unlocks the adopted worktree, asks OpenCode to remove its native workspace, and records completion. A failed removal is persisted as pending and the still-registered worktree is locked again for a safe retry. OpenCode Reset is not an ITL close action and remains destructive for uncommitted changes.

OpenCode branch commands are fail-closed. Their first action is the managed workspace tool call; they must not load `1c-workflow` or search the repository for the tool implementation. If the tool is absent, they return `ITL_OPENCODE_WORKSPACE_TOOL_UNAVAILABLE`; repair with `/itl-update-workflow`, fully restart OpenCode Desktop, and retry the original branch command.
