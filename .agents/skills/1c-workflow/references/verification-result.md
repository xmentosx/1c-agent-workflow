...
The preferred 8.3.22 sequential `.lgp` reader decodes severity and identifiers from their fixed fields, resolves event and metadata names through `1Cv8.lgf`, and rejects non-`Error`/out-of-window events before signature construction. The seed baseline reads the number of recent source-log days configured by `SOURCE_EVENT_LOG_LOOKBACK_DAYS` (default `7`, any non-negative integer); `0` disables only this auxiliary historical baseline. A damaged or incomplete source segment is skipped with a warning while readable signatures are retained. Baselines cache per-segment signatures and a proven complete byte boundary under `.agent-1c/event-log-signature-cache/<source-key>.json`: unchanged segments are reused, append-only growth reads only the new tail, while truncation, replacement, incompatible/damaged cache, or an unsafe boundary forces a full segment scan. Canonical `check-dev-branch` preserves the oldest branch-persistent cursor before its owned base update, reuses a cursor left by a separate `update-dev-branch-base`, and copies that boundary into the Vanessa run evidence. The gate therefore covers config load/Enterprise normalization, delayed errors before the next check, and the current TestManager/TestClient cycle. Direct Vanessa-only runs keep a local cursor immediately before TestManager and do not consume the lifecycle cursor.
