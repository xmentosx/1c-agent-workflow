...
vibecoding1c MCP is exposed through `/itl-vibecoding1c-mcp`; setup selects if saved selection is missing/incomplete, and `-Force` reselects. Remote LAN endpoints are default. Config-specific remote MCP requires explicit `configId`. Developers may override each server to local; local `code`/`graph` needs project or branch scope. Vanessa MCP is separate branch-local tooling exposed through `/itl-vanessa-mcp`. External MCP is not managed. The helper may write ignored `.codex/config.toml`, `.kilo/kilo.json`, `.agent-1c/mcp/*`, and `%LOCALAPPDATA%\ITL\MCP\vibecoding1c` state. Do not paste license keys into chat or tracked files.
