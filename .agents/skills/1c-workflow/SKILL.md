...
For project initialization, first run the monitored helper wizard through `scripts/run-agent-1c-window.ps1 -- -Action init-project -InitMode wizard`. The wizard collects setup values, writes `.dev.env` and `.agent-1c/project.json`, summarizes values without passwords, and then runs the initialization lifecycle. The monitored launcher writes `.agent-1c/runs/<run>/status.json` and `console.log`, so the agent can detect completion without asking the developer to close the wizard window manually. Run the monitored command in the foreground and wait for it to exit; do not wrap it in a background PowerShell launch, do not keep the launched PowerShell session open after the helper exits, and do not call `agent-1c.ps1 -Action init-project -InitMode wizard` directly as the default agent path. Do not run a separate `Test-Path` preflight before the launcher; the launcher validates the helper path itself, and extra PowerShell probes can emit serialized `CLIXML` progress noise. If the agent shell tool accepts a timeout, set a positive long timeout such as `1800000` ms for the interactive wizard; never use `timeout: 0` or an infinite timeout sentinel. Use `-KeepWindowOnFailure` only for manual debugging when the developer explicitly wants the external PowerShell window to stay open after a failure. Do not collect the initialization questionnaire in chat or Kilo Questions before this first helper attempt. If the wizard fails because terminal input is unavailable, do not collect the questionnaire in chat and do not continue the lifecycle manually. Use the monitored wizard command, or use `-InitMode json -InitAnswersPath <file>` only when the developer explicitly requested non-interactive initialization or an answers file already exists.
