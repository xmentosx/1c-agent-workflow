...
2. If configuration or extension sources changed since the last branch-infobase load, run the supported `update-dev-branch-base` helper action first. Its fingerprint check skips Designer/Enterprise when the infobase is already current; a `.feature`-only edit needs no base update. If the facade still returns `ITL_INFOBASE_APPLICATION_NOT_READY`, run that helper once and repeat the original MCP call once without asking the user.
3. Call the pre-registered `itl-vanessa-ui` MCP server. It exposes compact `resolve_tool`, `call_tool`, and `finish_database_access`; the verified 38-tool catalog stays inside the facade. For a known parameterized semantic tool, call `call_tool` directly with its exact inner `name` and `argumentsJson` containing one JSON-encoded object with only explicitly intended fields; this preserves arbitrary inner property names across schema-restricting clients. For a no-argument inner tool, use `arguments={}`. Never send both forms. Use `resolve_tool` once only when the name or schema is unknown; resolution does not start Vanessa or 1C.
...
5. Use its semantic tools only to answer the recorded runtime question. The backend and its owned TestClient may stop after ten minutes without completed calls, but the agent-owned database phase remains retained until `finish_database_access` succeeds. Client exit performs ownership-scoped abandonment cleanup.
...
Before an incompatible lifecycle, test, or measurement phase, explicitly choose one path: continue this task's current UI phase and postpone the next phase, or finish the UI phase and call `finish_database_access` on the same `itl-vanessa-ui` facade. Wait for that call to confirm release before starting the incompatible phase. For an interactive profile owned outside this facade, use its existing exact `stop-vanessa-profile` action instead.

Release only access owned by the current task and exact facade/profile instance. Never finish or stop a foreign holder; report its owner as the blocker and leave its TestClient, process, and lease intact. Idle timeout or client exit is an abandonment fallback, not a normal handoff.
...
`run_scenario` progress is conditional. Vanessa emits `step N/M` notifications, and the facade forwards them only when the calling MCP client supplied a `progressToken`; the client must also expose those notifications to the user. Without that metadata, no intermediate message is promised. The Vanessa backend uses one UI session, so a concurrent `get_VanessaAutomation_state` can wait behind the active scenario and is not an independent liveness probe. Set the caller timeout from the expected scenario duration; silence alone is not a hang. `progressTokenProvided` identifies missing caller metadata. `progressNotificationsForwarded` is the count at outcome recording; asynchronous handlers can finish later. Correlate `progressEvidenceId` with `<instance>.progress.jsonl` for successful forwarding events, including late notifications; do not infer absent upstream progress solely from a zero completion snapshot.
