...
5. Use its semantic tools only to answer the recorded runtime question. The backend and its owned TestClient may stop after ten minutes without completed calls. Database ownership is call-scoped and ends only after the result and bounded owned cleanup; idle time between calls owns no database resource.
...
Do not create or finish a retained UI database phase. A conflicting lifecycle, test, or measurement execution waits on the exact call-scoped guard and proceeds automatically after owned cleanup. For an interactive profile owned outside this facade, use its existing exact `stop-vanessa-profile` action instead.
