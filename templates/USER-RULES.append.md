...
Do not use `/deploy-and-test` as the normal verification command in an ITL development branch because it reloads all files. The normal post-change executable cycle is `/itl-check`; it updates the branch infobase and runs Vanessa Automation. `/itl-verify` remains a compatibility alias. Use `/itl-update-base` only when you need to update the branch infobase without tests.
