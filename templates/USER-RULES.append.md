...
`ITL_YAXUNIT_TESTING`, `ITL_VANESSA_TESTING`, `ITL_CHECK_EVENT_LOG`: `auto|manual|off`, default `auto`. Before changing non-trivial algorithms or recovery, read `yaxunit-tests.md`. Require YAxUnit: normal, empty/minimum, below/at/above boundary, dangerous negative/extreme cases in parameterized tables. Integration/UI needs Vanessa; complex changes may need both. Reuse only tests proving change.
