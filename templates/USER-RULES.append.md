...
Before incompatible database work, either retain owned activity and postpone, or finish it through its exact owner; never await its idle timeout. Let bounded jobs finish or cancel them through their owner. Never release foreign work. A helper result with `errorCategory=database-access-blocked` is a safety/ownership continuation, not a workflow defect. For `agent-owned-handoff-required`, execute only the exact reported `blockerAction` on its owning surface and retry the original task. For `user-decision-or-external-action`, ask only for the stated decision or external action; after confirmed release, resume and retry the original task. Never force-unlock, edit coordinator state, weaken the guard, or route these blockers to generic repair.
