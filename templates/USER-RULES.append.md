...
Before changing product business logic, architecture, user workflows, terminology, permissions, reports, integrations, or acceptance tests, use `.agents/skills/product-docs/SKILL.md` and search `BookStack-product-docs-mcp`. BookStack is advisory, not authoritative; verify against code, tests, current 1C metadata, and available MCP evidence. Cite relevant BookStack page URLs/`updated_at` in plans, explanations, review notes, and PR summaries. If BookStack and implementation disagree, explicitly report `BookStack says`, `Code/MCP currently shows`, and `Decision` instead of silently choosing one source.
