...
Before changing product business logic, architecture, user workflows, terminology, permissions, reports, integrations, or acceptance tests, use `.agents/skills/product-docs/SKILL.md` and search `BookStack-product-docs-mcp`. Cite relevant BookStack page URLs in plans, explanations, review notes, and PR summaries. If BookStack and code disagree, explicitly report the conflict instead of silently choosing one source.
