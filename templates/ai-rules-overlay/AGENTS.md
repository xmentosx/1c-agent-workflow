...
### Triage: task kind, execution path, and planning mode
...
4. **Full-cycle:** everything else or any doubt. Full-cycle is not OpenSpec and does not select a planning mode. Delegation is optional and governed by `content/rules/subagents.md`.

For code/metadata implementation, record independent `executionPath=quick-fix|full-cycle` and `planningMode=direct|OpenSpec`; all four combinations are valid. Default `direct`; use OpenSpec on request or for valuable formal agreement. Planning mode never changes verification depth. Promotion triggers set only `executionPath=full-cycle`.
...
- BSL uses the applicable syntax, logic/review, style, impact, and runtime gates from `verification-gates.md`. `VERIFICATION_DEPTH=standard` is the default and requires the same three kinds of static evidence as `full`; direct-mode `check_1c_code` may satisfy syntax and logic in one call when its explicit syntax section is clean. Defer AI validators to the closing coherent snapshot, and run `review_1c_code` only after implementation and logic validation are complete. Promotion-trigger paths use the full post-fix budget. Metadata XML uses schema/examples plus `verify_xml`. Embedded BSL requires both chains.
