...
- BSL uses the applicable syntax, logic/review, style, impact, and runtime gates from `verification-gates.md`. `VERIFICATION_DEPTH=standard` is the default and requires the same three validators as `full`: obtain standalone `syntaxcheck` / `syntaxcheck_file` evidence first, then use `check_1c_logic` for logic/performance when exposed (`check_1c_code` is the compatibility fallback), and run `review_1c_code` last. Defer AI validators to the closing coherent snapshot. Promotion-trigger paths use the full post-fix budget. Metadata XML uses schema/examples plus `verify_xml`. Embedded BSL requires both chains.
